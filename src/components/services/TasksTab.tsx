    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-in fade-in duration-300">
      {activeTasks.length > 0 ? (
        activeTasks.map(task => {
          ...
          return (
            <div key={task.id} className="data-card p-4 flex flex-col justify-between hover:border-[#66B2B2]/40 transition-all cursor-pointer group" onClick={() => draftExecutions[task.id]?.travelStartTime ? onContinueTask(task) : draftExecutions[task.id]?.travelStartTime ? onContinueTask(task) : onStartTask(task)}>
               <div>
