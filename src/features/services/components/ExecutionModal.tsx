...
import { useInventoryStore } from "@/features/inventory/useInventoryStore";
...
export function ExecutionModal({
  ...
  const completeSeedServiceRecordMutation =
    trpc.seedServiceRecords.complete.useMutation();
  const trpcUtils = trpc.useUtils();

  const draft = task
    ? draftExecutions[task.id] || { currentStep: 1, partsUsed: "Pending" }
    : null;
  ...
  const handleScanSuccess = async (decodedText: string) => {
    const currentEq = equipment.find(
      e => e.id === (draft?.equipmentId || task?.equipmentId)
    );
    let _scanMeta: any = {};
    try {
      _scanMeta = JSON.parse(task?.description ?? "{}");
    } catch {}
    const _scanSeedEq =
      _scanMeta._src === "pms"
    ...
    const expectedSerial =
      _scanSeedEq?.serialNumber ?? currentEq?.serialNumber ?? "";
    if (decodedText.trim() === expectedSerial) {
      await stopScanning();
      setIsVerified(true);
      toast.success("Asset Verified Successfully!");
      if (task) {
        updateDraftExecution(task.id, {
  ...
  const submitFinalReport = () => {
    ...
    try {
      ...
      const snapshotInterval: number | undefined =
        meta._serviceInterval !== undefined
          ? Number(meta._serviceInterval)
      ...
      if (pmsCfg || snapshotIntervalUnit !== undefined) {
        ...
        try {
          gps001Ms =
            Number(
              window.localStorage.getItem("nextos-gps001-total-hours-ms") ?? "0"
            ) || 0;
        } catch {}
      ...
      if (seedEq && snapshotInterval && snapshotIntervalUnit) {
        ...
        if (_unit === "hours") {
          if (seedEq?.id === "EQ-001") {
            ...
            try {
              _rawMs =
                Number(
                  window.localStorage.getItem("nextos-gps001-total-hours-ms") ??
                    "0"
                ) || 0;
            } catch {}
            try {
              _offsetMs =
                Number(
                  window.localStorage.getItem(GPS001_HOURS_OFFSET_KEY) ?? "0"
                ) || 0;
            } catch {}
            const _effectiveMs = Math.max(0, _rawMs - _offsetMs);
            _usage = _effectiveMs / (1000 * 60 * 60);
          } else {
            const _m = String(seedEq.hoursTotal ?? "").match(
              /(\d+)\s*h\s*(\d+)\s*m/i
            );
          ...
          const _p =
            typeof _raw === "number"
              ? _raw
              : parseFloat(String(_raw ?? "").replace(/[^\d.]/g, ""));
          if (Number.isFinite(_p) && _p >= 0) _usage = _p;
        } else {
          const _days =
            typeof seedEq.days === "number"
              ? seedEq.days
              : parseFloat(String(seedEq.days ?? ""));
        ...
        if (_usage !== null && Number.isFinite(_usage) && _usage >= 0) {
          ...
          computedEquipmentStatus =
            _pct >= 100 ? "Overdue" : _pct >= 80 ? "Near Service" : "OK";
        }
      }
      ...
      const submissionPayload = {
        ...
        equipmentId:
          Number(String(effectiveEquipId ?? "").replace(/\D/g, "")) || 0,
        ...
        clientName:
          seedClient?.companyName ??
          clients.find(c => c.id === task.clientId)?.companyName ??
          "",
      ...
      completeSeedServiceRecordMutation.mutate(submissionPayload, {
        onSuccess: () => {
          ...
          if (shouldResetMetrics) {
            onMetricsReset?.(
              meta._seedEqId as string,
              pmsCfg.serviceIntervalUnit ?? "Hours"
            );
          }
        },
        onError: () => {
          ...
          toast.error(
            "Couldn't write report to records — will retry automatically."
          );
        },
      });
    } catch {
      /* silently skip — in-memory update already done */
    }
  ...
  useEffect(() => {
    return () => {
      if (scannerRef.current) {
        try {
          const s = scannerRef.current.stop();
          if (s && typeof (s as any).catch === "function")
            (s as any).catch(() => {});
        } catch {}
        try {
          const c = scannerRef.current.clear();
          if (c && typeof (c as any).catch === "function")
            (c as any).catch(() => {});
        } catch {}
      }
    };
  ...
  let _pmsMeta: any = {};
  try {
    _pmsMeta = JSON.parse(task?.description ?? "{}");
  } catch {}
  ...
  const displaySerial =
    _pmsSeedEq?.serialNumber ?? currentEq?.serialNumber ?? "";
  const displayName =
    _pmsSeedEq?.name ??
    currentEq?.name ??
    currentEq?.id ??
  ...
  const displaySubtitle =
    _pmsSeedEq?.equipmentType ?? currentEq?.equipmentType ?? "";
  const displayClient = _pmsSeedEq?.clientId
    ? (seedClients.find((c: any) => c.id === _pmsSeedEq.clientId)
        ?.companyName ?? null)
    : null;
  ...
  const displayOperatingTime = (() => {
    if (_pmsSeedEq?.id === "EQ-001") {
      try {
        ...
        const offsetMs =
          Number(window.localStorage.getItem(GPS001_HOURS_OFFSET_KEY) ?? "0") ||
          0;
          ...
          <div className="flex flex-col h-full">
            <DialogHeader className="p-6 border-b border-gray-50 bg-gray-50/30 rounded-t-2xl">
              <DialogTitle className="flex items-center gap-2 text-gray-900 text-lg font-bold">
                ...
                Service Execution:{" "}
                <span className="font-mono-tech">{displayName}</span>
              </DialogTitle>
              <DialogDescription className="text-xs text-gray-500 mt-1">
                Complete the following steps to document and finalize the
                service execution for this asset.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="px-10 py-6 border-b border-gray-50">
                  ...
                  >
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold z-10 transition-all ${
                        currentStep === step
                          ? "bg-[#66B2B2] text-white ring-4 ring-[#66B2B2]/10 scale-110"
                          : currentStep > step
                    ...
                    {i < 5 && (
                      <div
                        className={`h-1 flex-1 mx-2 rounded-full ${currentStep > step ? "bg-green-500" : "bg-gray-100"}`}
                      />
                    )}
                  </div>
                ))}
              ...
              <div className="flex justify-between mt-3 px-1">
                {["Scan", "Safety", "Before", "Findings", "After", "Sign"].map(
                  (label, i) => (
                    <span
                      key={label}
                      className={`text-[9px] font-bold uppercase tracking-wider ${currentStep === i + 1 ? "text-[#66B2B2]" : "text-gray-400"}`}
                    >
                      {label}
                    </span>
                  )
            ...
            <div className="p-8 flex-1">
                ...
                <div className="space-y-6 animate-in fade-in duration-300">
                  <div className="text-center pb-2">
                    ...
                    <p className="text-sm text-gray-500">
                      Confirm you are at the correct unit before beginning
                      documentation.
                    </p>
                  </div>
                  ...
                  {currentEq ? (
                      ...
                      {isScanning ? (
                        <div className="space-y-4">
                          <div className="text-center">
                            <div className="inline-flex items-center gap-2 px-3 py-1 bg-[#66B2B2]/20 text-[#66B2B2] rounded-full text-[10px] font-black uppercase tracking-[0.1em] mb-4">
                              <div className="w-2 h-2 rounded-full bg-[#66B2B2] animate-ping" />{" "}
                        ...
                        <div className="py-12 text-center space-y-4 animate-in zoom-in-95 duration-500">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-400">
                              Lock-on confirmed. Initializing service report...
                            </p>
                          </div>
                        </div>
                      ) : (
                        <>
                          <div className="flex items-start justify-between">
                            <div>
                              <div className="text-[10px] text-[#66B2B2] font-black uppercase tracking-[0.2em] mb-1">
                                Target Asset
                              </div>
                              ...
                              {displayClient && (
                                <div className="text-xs text-[#66B2B2] font-semibold mt-0.5">
                                  {displayClient}
                                </div>
                              )}
                          ...
                          <div className="grid grid-cols-2 gap-6 py-6 border-y border-white/5">
                            <div className="space-y-1">
                              ...
                              <div className="text-sm font-mono-tech font-bold text-[#66B2B2]">
                                {displaySerial}
                              </div>
                            </div>
                    ...
                    <div className="p-10 text-center bg-gray-50 rounded-2xl border border-dashed">
                      ...
                      <p className="text-sm text-gray-500 font-bold">
                        Error: Asset data not found for this task.
                      </p>
                    </div>
                  )}
                </div>
              ...
              {currentStep === 2 && (
                <SafetyProtocol
                  ...
                  onSave={checklist =>
                    handleNext({ safetyChecklist: checklist })
                  }
                  onBack={handleBack}
                />
              )}
              ...
              {currentStep === 3 && (
                <div className="space-y-6 animate-in fade-in duration-300">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-gray-500">
                      Capture the initial condition of the asset.
                    </p>
                  </div>
                  <VisualEvidence
                    label="BEFORE"
                    ...
                    onSave={(photo, notes) =>
                      handleNext({ beforePhoto: photo, beforeNotes: notes })
                    }
                    onBack={handleBack}
                  />
                </div>
              ...
              {currentStep === 5 && (
                <div className="space-y-6 animate-in fade-in duration-300">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-gray-500">
                      Capture the asset state after service completion.
                    </p>
                  </div>
                  <VisualEvidence
                    label="AFTER"
                    ...
                    onSave={(photo, notes) =>
                      handleNext({ afterPhoto: photo, afterNotes: notes })
                    }
                    onBack={handleBack}
                  />
                </div>
              ...
              {currentStep === 6 && (
                <div className="space-y-8 animate-in fade-in duration-300">
                  <SignaturePad
                    ...
                    onChange={sig =>
                      updateDraftExecution(task.id, { techSignature: sig })
                    }
                  ...
                  <SignaturePad
                    ...
                    onChange={sig =>
                      updateDraftExecution(task.id, { clientSignature: sig })
                    }
