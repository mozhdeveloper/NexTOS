...
import { getPmsMetricValue, getPmsMetricLabel } from "../service.utils";
...
export function TechnicalWorkForm({
  draft,
  equipment,
  client,
  packages,
  ...
  const [fields, setFields] = useState({
    ...
    hoursAtService:
      ...
      Math.floor(
        parseFloat((equipment?.hoursTotal ?? "0").match(/(\d+)/)?.[1] ?? "0")
      ) ||
      0,
    cost: draft.cost ?? 0,
  });
  ...
  const clientName = useMemo(() => {
    if (seedEquipment?.clientId) {
      const sc = (seedClients ?? []).find(
        (c: any) => c.id === seedEquipment.clientId
      );
  ...
  const eqType =
    seedEquipment?.equipmentType ?? equipment?.equipmentType ?? "—";
      ...
      <div className="grid grid-cols-1 gap-4">
        <div className="p-4 rounded-xl bg-gray-50 border border-gray-100">
          ...
          <div className="text-xs text-white mt-1">
            <span className="font-semibold text-gray-600">Equipment Name:</span>{" "}
            {eqName}
          </div>
          <div className="text-xs text-white mt-1">
            <span className="font-semibold text-gray-600">Equipment Type:</span>{" "}
            {eqType}
          </div>
        ...
        <div className="p-4 rounded-xl bg-gray-50 border border-gray-100">
          <div className="text-[10px] text-white uppercase font-bold tracking-wider mb-2">
            Service Context
          </div>
          {pmsConfig ? (
      ...
      <div className="grid gap-4">
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            onChange={e =>
              setFields({ ...fields, recommendations: e.target.value })
            }
          />
        </div>
          ...
          <div className="flex gap-2 items-end">
            ...
            <div className="w-24 space-y-1">
              <label className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">
                Qty{" "}
                {selectedPartId !== ""
                  ? `(${inventoryItems.find(i => i.id === selectedPartId)?.unit ?? ""})`
                  : ""}
              </label>
              <Input
                type="number"
            ...
            <Button
              ...
              disabled={
                selectedPartId === "" || !partQty || Number(partQty) <= 0
              }
              onClick={() => {
          ...
          {(fields.selectedParts ?? []).length > 0 && (
            <div className="space-y-1.5 mt-1">
              {(fields.selectedParts ?? []).map(part => (
                ...
                >
                  <div className="text-xs font-semibold text-gray-700 flex-1 min-w-0 truncate">
                    {part.name}
                  </div>
                  <div className="text-[10px] text-gray-400 mx-3 shrink-0">
                    {part.quantity} × ₱
                    {part.pricePerUnit.toLocaleString("en-PH")}
                  </div>
                  <div className="text-xs font-bold text-[#66B2B2] shrink-0 mr-2">
                    ₱
                    {(part.quantity * part.pricePerUnit).toLocaleString(
                      "en-PH",
                  ...
                  <button
                    ...
                    onClick={() =>
                      setFields({
                        ...fields,
                        selectedParts: (fields.selectedParts ?? []).filter(
                          p => p.inventoryItemId !== part.inventoryItemId
                        ),
                      })
                    }
                  >
