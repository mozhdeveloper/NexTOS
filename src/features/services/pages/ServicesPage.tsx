...
import { ServiceReportsTab } from "@/features/services/components/ServiceReportsTab";
import { TasksTab } from "@/features/services/components/TasksTab";
import { ManualLogTab } from "@/features/services/components/ManualLogTab";
import { EquipmentTab } from "@/features/services/components/EquipmentTab";
import { ScheduledMaintenanceTab } from "@/features/services/components/ScheduledMaintenanceTab";
import { ServiceReportModal } from "@/features/services/components/ServiceReportModal";
import { QrScannerModal } from "@/features/services/components/QrScannerModal";
import { QrTagModal } from "@/features/services/components/QrTagModal";
import { PreServiceConfirmModal } from "@/features/services/components/PreServiceConfirmModal";
import { ExecutionModal } from "@/features/services/components/ExecutionModal";
import { ServicesHeader } from "@/features/services/components/ServicesHeader";
import { useGps001Cache } from "@/features/fleet/hooks/useGps001Cache";
import { useMetricsOverrides } from "@/features/fleet/hooks/useMetricsOverrides";

import type {
  ScheduledMaintenanceEntry,
  TabType,
...
function mapPmsServiceCategory(
  serviceType?: string,
  ...
  if (
    st.includes("pms") ||
    st.includes("preventative") ||
    st.includes("preventive")
  )
  ...
  try {
    const raw =
      typeof window !== "undefined"
        ? window.localStorage.getItem(SCHEDULED_MAINTENANCE_KEY)
        : null;
...
export default function Services() {
  ...
  const [activeTab, setActiveTabRaw] = useState<TabType>(() => {
    try {
      return (
        (sessionStorage.getItem("nextos-services-tab") as TabType) ?? "tasks"
      );
    } catch {
      return "tasks";
    }
  ...
  const setActiveTab = (tab: TabType) => {
    try {
      sessionStorage.setItem("nextos-services-tab", tab);
    } catch {}
    setActiveTabRaw(tab);
  };
  ...
  // Modal State
  const [executionTask, setExecutionTask] = useState<ServiceRecord | null>(
    null
  );
  const [confirmTask, setConfirmTask] = useState<ServiceRecord | null>(null);
  const [showReport, setShowReport] = useState<
    (ServiceRecord & Record<string, any>) | null
  >(null);
  ...
  const [userScheduledMaintenance, setUserScheduledMaintenance] = useState<
    ScheduledMaintenanceEntry[]
  >(readUserScheduledMaintenance);
  const [editModalOpen, setEditModalOpen] = useState(false);
  const [editingEntry, setEditingEntry] =
    useState<ScheduledMaintenanceEntry | null>(null);
  ...
  const addPmsConfigurationMutation =
    trpc.seedEquipment.addPmsConfiguration.useMutation();
  const updatePmsConfigurationMutation =
    trpc.seedEquipment.updatePmsConfiguration.useMutation();
  const deletePmsConfigurationMutation =
    trpc.seedEquipment.deletePmsConfiguration.useMutation();
  ...
  const { data: seedServiceRecordsData } =
    trpc.seedServiceRecords.list.useQuery(undefined, {
      staleTime: 60_000,
    });
  ...
  const { data: seedEquipmentQueryData } = trpc.seedEquipment.list.useQuery(
    undefined,
    {
      staleTime: 10_000,
  ...
  const liveEquipment: any[] =
    seedEquipmentQueryData?.equipment ?? (seedData.equipment as any[]);
  const liveClients: any[] =
    seedEquipmentQueryData?.clients ?? (seedData.clients as any[]);
  ...
  const activeTasks = serviceRecords.filter(
    r => r.status === "scheduled" || r.status === "in_progress"
  );

  const filteredSeedEquipment = liveEquipment.filter(
    s =>
      searchQuery === "" ||
  ...
  const computeEntryStatus = (
    ...
    if (unit === "hours") {
      if (eq.id === "EQ-001" && gps001CacheMs > 0) {
        ...
        const match = String(effectiveEq.hoursTotal ?? "").match(
          /(\d+)\s*h\s*(\d+)\s*m/i
        );
    ...
    const match = seedData.pmsStatuses.find(
      s => progress >= s.minProgressPercent
    );
  ...
  const computeEquipmentWorstStatus = (
    seedEq: any
  ): "OK" | "Near Service" | "Overdue" | null => {
    if (!seedEq) return null;
    const configs: any[] = Array.isArray(seedEq.pmsConfiguration)
      ? seedEq.pmsConfiguration
      : [];
  ...
  const seedScheduledMaintenance = useMemo((): ScheduledMaintenanceEntry[] => {
    ...
    liveEquipment.forEach(eq => {
      const configs: any[] = Array.isArray(eq.pmsConfiguration)
        ? eq.pmsConfiguration
        : [];
      ...
      configs.forEach((pms: any, index: number) => {
        entries.push({
          id:
            index === 0 ? `seed-pms-${eq.id}` : `seed-sched-${eq.id}-${index}`,
          equipmentId: eq.id,
          scheduleIndex: index,
  ...
  const allScheduledMaintenance = useMemo(() => {
    ...
    const extraUserEntries = userScheduledMaintenance.filter(
      e => !seedIds.has(e.id)
    );
  ...
  useEffect(() => {
    if (!seedServiceRecordsData?.records || seedRecordsInjectedRef.current)
      return;
    ...
    const seedCompletedIds = new Set(
      seedServiceRecordsData.records
    ...
    useOperationsStore.setState(state => ({
      serviceRecords: state.serviceRecords.filter(r => {
        ...
        const isPms =
          ...
          (() => {
            try {
              return JSON.parse(r.description ?? "{}")._src === "pms";
            } catch {
              return false;
            }
          })();
      ...
      useOperationsStore.setState(state => {
        ...
        return {
          serviceRecords: [
            ...
            {
              ...record,
              equipmentId: record.seedEquipmentId ?? String(record.equipmentId),
              invoiceId: null,
              createdAt: new Date().toISOString(),
              serviceCategory: record.serviceCategory as any,
            },
          ],
        };
      });
  ...
  useEffect(() => {
    ...
    const overdueEntries = allScheduledMaintenance.filter(
      e => e.status === "Overdue"
    );
    ...
    for (const entry of overdueEntries) {
      ...
      if (liveRecords.some(r => r.id === taskId && r.status !== "completed"))
        continue;
      ...
      const serviceCategory = mapPmsServiceCategory(
        entry.serviceType,
        seedEq.equipmentType
      );
        ...
        serviceRecords: [
          ...state.serviceRecords.filter(r => r.id !== taskId),
          {
            ...
            hoursAtService:
              Math.floor(
                parseFloat(
                  (storeEq.hoursTotal ?? "0").match(/(\d+)/)?.[1] ?? "0"
                )
              ) || 0,
            invoiceId: null,
            createdAt: new Date().toISOString(),
      ...
      if (!alreadyCompletedInSeed) {
        upsertSeedServiceRecord.mutate({
          ...
          hoursAtService:
            Math.floor(
              parseFloat((storeEq.hoursTotal ?? "0").match(/(\d+)/)?.[1] ?? "0")
            ) || 0,
        });
      }
  ...
  const handleDeleteEntry = async (entry: ScheduledMaintenanceEntry) => {
    ...
    if (seedEq && entry.scheduleIndex !== undefined) {
      try {
        ...
        toast.success(
          `Deleted "${entry.serviceType}" schedule for ${entry.equipmentName}.`
        );
      } catch {
  ...
  const handleScheduleSubmit = async () => {
    ...
    if (!scheduleInterval || Number(scheduleInterval) <= 0)
      missing.push("serviceInterval");
    if (missing.length > 0) {
    ...
    const serviceTypeLabel =
      serviceTypeOptions.find(opt => opt.value === scheduleServiceType)
        ?.label ?? scheduleServiceType;
    const numericInterval = Number(scheduleInterval);
    const intervalUnit = scheduleIntervalUnit as
      | "Hours"
      | "KM"
      | "Weeks"
      | "Months"
    ...
    if (unitTaken) {
      toast.error(
        `This equipment already has a ${intervalUnit} schedule. Delete the existing one first to add a new one.`
      );
      return;
    }

    // Always append to the pmsConfiguration array — there is no "primary vs additional" distinction
    let optimisticConfigIndex = Array.isArray(seedEq.pmsConfiguration)
      ? seedEq.pmsConfiguration.length
      : 0;
    ...
    try {
      ...
      toast.error(
        "Failed to write to seed data. Changes are saved locally only."
      );
    }
    ...
    const existingIdx = userScheduledMaintenance.findIndex(
      e => e.id === optimisticId
    );
    const next =
      existingIdx >= 0
        ? userScheduledMaintenance.map((e, i) =>
            i === existingIdx ? optimisticEntry : e
          )
  ...
  const openEditModal = (entry: ScheduledMaintenanceEntry) => {
    ...
    const matchingOption =
      serviceTypeOptions.find(
        opt =>
          opt.value === entry.serviceType || opt.label === entry.serviceType
      ) ??
      (entry.serviceType === "Heavy Equipment PMS"
    ...
    setEditEstimatedCost(
      entry.estimatedCost > 0 ? String(entry.estimatedCost) : ""
    );
  ...
  const handleEditSubmit = async () => {
    ...
    if (!editInterval || Number(editInterval) <= 0)
      missing.push("serviceInterval");
    if (missing.length > 0) {
    ...
    const serviceTypeLabel =
      serviceTypeOptions.find(opt => opt.value === editServiceType)?.label ??
      editServiceType;
    const numericInterval = Number(editInterval);
    const intervalUnit = editIntervalUnit as
      | "Hours"
      | "KM"
      | "Weeks"
      | "Months"
    ...
    if (unitTakenByOther) {
      toast.error(
        `This equipment already has a ${intervalUnit} schedule. Change or delete that entry first.`
      );
      return;
    }
  ...
  const handleManualLogSubmit = () => {
    if (!formClientId || !formEquipmentId || !formWorkDone) {
      toast.error("Required documentation missing", {
        description:
          "Please ensure Client, Equipment, and Technical Work sections are filled.",
      });
      return;
    }
    if (!formTechSign || !formClientSign) {
      toast.error("Signatures Required", {
        description: "Both Technician and Client signatures must be captured.",
      });
      return;
    }
    ...
    addServiceRecord({
      ...
      hoursAtService:
        Math.floor(
          parseFloat((targetEq?.hoursTotal ?? "0").match(/(\d+)/)?.[1] ?? "0")
        ) || 0,
    ...
    if (formAfterPhoto) {
      addServicePhoto({
        ...
        type: "after",
        url: formAfterPhoto,
        caption: "Manual Log: After",
      });
    }

    // Persist to seed data
    try {
      const seedEq = liveEquipment.find(
        (s: any) => s.serialNumber === targetEq?.serialNumber
      );
      const seedClient = liveClients.find(
        (c: any) => c.id === parseInt(formClientId)
      );
    ...
    toast.success("Manual Report Sealed", {
      description: "Service record has been officially added to history.",
    });
  ...
  return (
    <div className="space-y-4">
      ...
      <QrTagModal
        ...
        qrSerial={qrSerial}
        unitName={
          equipment.find(e => e.serialNumber === qrSerial)?.name ??
          equipment.find(e => e.serialNumber === qrSerial)?.id ??
          ""
        }
      />
    </div>
