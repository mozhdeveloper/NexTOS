...
export default function DashboardPage() {
  ...
  const totalOverdueEquipment = equipment.filter(
    e => e.status === "service_due"
  ).length;
  ...
  const topTech = useMemo(() => {
    const techStats: Record<
      string,
      { name: string; jobs: number; safety: number; safetyCount: number }
    > = {};
    serviceRecords.forEach(s => {
      if (s.status !== "completed") return;
      if (!techStats[s.technician]) {
        techStats[s.technician] = {
          name: s.technician,
          jobs: 0,
          safety: 0,
          safetyCount: 0,
    ...
    const techs = Object.values(techStats)
      .map(t => ({
        ...
        powerScore:
          (t.safetyCount > 0 ? (t.safety / t.safetyCount) * 0.6 : 60) +
          Math.min(t.jobs, 10) * 4,
      }))
  ...
  const funnelData = [
    {
      ...
      value:
        ...
        deals.filter(d => d.stage === "inquiry").length,
    },
    {
      name: "Proposal",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        ...
        <div className="data-card p-4">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-gray-500">
                Operating hours vs target by unit
              </p>
            </div>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart
              data={units.slice(0, 5).map(u => ({
                name: u.unitName.split("-")[1] || u.unitName,
                hours: Math.floor(u.telemetry.hours),
                target: 160,
              }))}
              barGap={4}
            >
        ...
        <div className="data-card p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[10px] font-black text-[#66B2B2] bg-[#66B2B2]/5 px-2 py-0.5 rounded-full uppercase tracking-tighter">
              Live Updates
            </span>
          </div>
            ...
            {serviceRecords
              ...
              .map((record, i) => (
                <div key={record.id} className="flex gap-3 relative">
                  {i < 4 && (
                    <div className="absolute left-[13px] top-7 bottom-0 w-[1px] bg-gray-100" />
                  )}
                  <div
                  ...
                  <div>
                    <div className="text-[11px] font-bold text-gray-900">
                      {record.technician}{" "}
                      {record.status === "completed" ? "finalized" : "started"}{" "}
          ...
          <div className="overflow-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {equipment
                  ...
                  .map(eq => (
                    ...
                    >
                      ...
                      <td className="py-2 font-mono-tech font-bold text-gray-700">
                        {eq.hoursTotal ?? "—"}
                      </td>
                      <td className="py-2 font-mono-tech text-gray-400">
                        {eq.pmsConfiguration?.[0]?.serviceInterval ?? "—"}h
                      </td>
                      <td className="py-2">
                        <span
                          className={`px-1.5 py-0.5 rounded-[4px] text-[9px] font-black uppercase tracking-tighter ${
                ...
                {equipment.filter(e => e.status === "service_due").length ===
                  0 && (
                  <tr>
                    <td
                      colSpan={4}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
          ...
          <div className="space-y-3">
            {lowStockItems.length > 0 ? (
              lowStockItems.slice(0, 3).map(item => (
                <div
                  key={item.id}
                  className="p-3 rounded-xl bg-white border border-amber-100 shadow-sm flex items-center justify-between"
                >
                  <div>
                    <div className="text-[11px] font-bold text-gray-900">
                      {item.name}
                    </div>
                    <div className="text-[9px] text-gray-400 font-mono-tech">
                      {item.partNumber}
                    </div>
                  </div>
        ...
        <div className="data-card p-4 relative overflow-hidden bg-gradient-to-br from-amber-500/5 to-transparent border-amber-200">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 text-[9px] font-black uppercase tracking-tighter">
              Elite Performer
            </span>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                <div className="text-[10px] text-gray-400 font-bold uppercase">
                  {topTech.jobs} Jobs Completed
                </div>
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map(i => (
                    <Star
                      key={i}
                      className={`w-2 h-2 ${i <= 5 ? "text-amber-500 fill-amber-500" : "text-gray-200"}`}
                    />
                  ))}
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-3">
        {units.slice(0, 5).map(unit => {
          ...
          return (
            <div
              key={unit.id}
              className="data-card p-3 relative overflow-hidden group"
            >
              {unit.serviceDue && (
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                />
                <span className="text-xs text-gray-900 font-black truncate uppercase tracking-tight">
                  {unit.unitName}
                </span>
              </div>
              <div className="font-mono-tech text-[10px] text-gray-500 space-y-0.5">
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-900">
                    {unit.telemetry.lat.toFixed(3)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>LNG:</span>{" "}
                  <span className="text-gray-900">
                    {unit.telemetry.lng.toFixed(3)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>HRS:</span>{" "}
                  <span
                    className={`${unit.serviceDue ? "text-red-500 font-bold" : "text-[#66B2B2]"}`}
                  >
                    {Math.floor(unit.telemetry.hours)}
                  </span>
                </div>
                <div className="flex items-center justify-between pt-1 mt-1 border-t border-gray-50">
                  <span className="uppercase text-[8px] font-black text-gray-300">
                    Telemetry
                  </span>
                  <div className="flex items-center gap-1 text-gray-900 font-bold">
                    {unit.telemetry.speed}{" "}
                    <span className="text-[8px] text-gray-400">MPH</span>
                  </div>
                </div>
              </div>
            </div>
...
function KPICard({
  ...
  return (
    <div className={`data-card p-4 ${laser ? "laser-scan" : ""}`}>
      ...
      <div
        className={` text-4xl font-bold tracking-[-0.03em] mb-1 ${alert ? "text-[#EF4444]" : "text-gray-900 kpi-glow"}`}
      >
        {value}
      </div>
      <div className="flex items-center gap-1">
        {deltaUp ? (
          ...
          <ArrowDownRight
            className={`w-3 h-3 ${alert ? "text-[#EF4444]" : "text-[#66B2B2]"}`}
          />
        )}
        <span
          className={`text-xs font-medium ${alert ? "text-[#EF4444]" : deltaUp ? "text-[#10B981]" : "text-[#66B2B2]"}`}
        >
          {delta}
        </span>
      </div>
