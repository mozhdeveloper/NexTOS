...
export default function InventoryPage() {
  const { items, restockItem, addItem, updateItem, deleteItem } =
    useInventoryStore();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Warehouse Control & Spare Parts Logistics
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            className="bg-white text-gray-700 h-10 px-4 font-bold border-gray-200"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div
          className={`data-card p-5 border flex items-center gap-4 ${lowStockCount > 0 ? "bg-red-50 border-red-100" : "bg-white border-gray-100"}`}
        >
          <div
            className={`w-12 h-12 rounded-2xl flex items-center justify-center ${lowStockCount > 0 ? "bg-red-100" : "bg-green-50"}`}
          >
            <AlertTriangle
              className={`w-6 h-6 ${lowStockCount > 0 ? "text-red-600" : "text-green-600"}`}
            />
          </div>
          <div>
            <div className="text-[10px] text-gray-400 font-black uppercase tracking-widest">
            ...
            <div
              className={`text-2xl font-black ${lowStockCount > 0 ? "text-red-600" : "text-green-600"}`}
            >
              {lowStockCount}{" "}
        ...
        <div className="data-card p-5 bg-white border border-gray-100 flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-2xl font-black text-gray-900">
              ...
              {(
                items.reduce(
                  (sum, i) => sum + i.stockLevel * i.pricePerUnit,
                  0
                ) / 1000
              ).toFixed(1)}
              k
        ...
        <div className="data-card p-5 bg-white border border-gray-100 flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-2xl font-black text-gray-900">
              12 <span className="text-[10px] text-gray-400">Orders</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="data-card p-6 space-y-4">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              variant="outline"
              className="h-11 bg-white border-gray-200 font-bold px-4"
            >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50/50 border-y border-gray-100">
                <th className="text-left py-4 px-4 text-gray-400 font-black uppercase tracking-widest text-[10px]">
                  Part Identification
                </th>
                <th className="text-left py-4 px-4 text-gray-400 font-black uppercase tracking-widest text-[10px]">
                  Category
                ...
                <th className="text-right py-4 px-4 text-gray-400 font-black uppercase tracking-widest text-[10px]">
                  Unit Price
                </th>
                <th className="text-right py-4 px-4 text-gray-400 font-black uppercase tracking-widest text-[10px]">
                  Actions
                ...
                return (
                  ...
                  >
                    <td className="py-4 px-4">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-10 h-10 rounded-xl flex items-center justify-center ${isLow ? "bg-red-50 text-red-600" : "bg-[#66B2B2]/5 text-[#66B2B2]"}`}
                        >
                        ...
                        <div>
                          ...
                          <div className="text-[10px] text-gray-400 font-mono-tech tracking-wider uppercase">
                            {item.partNumber}
                          </div>
                        </div>
                      </div>
                    ...
                    <td className="py-4 px-4">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <div
                            className={`text-sm font-black ${isLow ? "text-red-600" : "text-gray-900"}`}
                          >
                    ...
                    <td className="py-4 px-4 text-right">
                      <div className="font-mono-tech font-bold text-gray-900">
      ...
      <Dialog
        open={!!showRestock}
        onOpenChange={open => !open && setShowRestock(null)}
      >
            ...
            <div className="space-y-6 pt-4">
              <div className="p-4 rounded-xl bg-[#66B2B2]/5 border border-[#66B2B2]/10">
                ...
                <div className="text-sm font-bold text-gray-900">
                  {items.find(i => i.id === showRestock)?.name}
                </div>
                <div className="text-[11px] text-gray-500 font-mono-tech mt-0.5">
                  {items.find(i => i.id === showRestock)?.partNumber}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-xs font-bold text-gray-400 uppercase">
                    {items.find(i => i.id === showRestock)?.unit} Incoming
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-lg font-black text-gray-900">
                    {(items.find(i => i.id === showRestock)?.stockLevel || 0) +
                      restockQty}{" "}
                    {items.find(i => i.id === showRestock)?.unit}
                  </div>
                </div>
                ...
                <div className="text-right">
                  ...
                  <div className="text-lg font-black text-[#66B2B2]">
                    ₱
                    {(
                      ((items.find(i => i.id === showRestock)?.stockLevel ||
                        0) +
                        restockQty) *
                      (items.find(i => i.id === showRestock)?.pricePerUnit || 0)
                    ).toLocaleString()}
                  </div>
                </div>
              </div>
      ...
      <EditPartModal
        ...
        part={
          editingPart !== null
            ? items.find(i => i.id === editingPart) || null
            : null
        }
        ...
        <DialogContent className="bg-white border-gray-200 sm:max-w-sm rounded-2xl shadow-2xl">
          ...
          <div className="py-6">
            <p className="text-gray-700 text-sm">
              ...
              want to delete this part entry?
            </p>
            {deletingPart && (
              <div className="mt-4 p-3 rounded-lg bg-gray-50 border border-gray-200">
                <div className="text-[10px] text-gray-600 uppercase font-bold tracking-wider mb-1">
                  Part to Delete
                </div>
                <div className="text-sm font-bold text-gray-900">
                  {items.find(i => i.id === deletingPart)?.name}
                </div>
                <div className="text-[10px] text-gray-600 mt-1">
                  {items.find(i => i.id === deletingPart)?.partNumber}
                </div>
              </div>
            )}
          </div>
