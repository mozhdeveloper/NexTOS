...
export default function CRM() {
  ...
  const [dealClientId, setDealClientId] = useState<number | null>(
    clients[0]?.id ?? null
  );

  // Deal action menu and modals
  const [openMenuDealId, setOpenMenuDealId] = useState<number | null>(null);
  const [selectedActionDeal, setSelectedActionDeal] = useState<Deal | null>(
    null
  );
  ...
  const [taskClientId, setTaskClientId] = useState<number | null>(
    clients[0]?.id ?? null
  );
  ...
  const clientData = useMemo(() => {
    ...
    return {
      ...
      serviceHistory: serviceRecords.filter(
      ...
      tasks: tasks.filter(
        t => t.relatedId === selectedClientId && t.relatedType === "client"
      ),
    };
  }, [
    selectedClientId,
  ...
  const handleSaveDealChanges = () => {
    ...
    useCRMStore.getState().updateDeal(selectedActionDeal.id, {
      ...
      probability:
        Number(editDealProbability) || selectedActionDeal.probability,
    });
    setEditDealOpen(false);
              ...
              {activeTab === "clients" && (
                <Select value={clientFilter} onValueChange={setClientFilter}>
                  ...
                  <SelectContent className="bg-white border-gray-200">
                    <SelectItem value="all" className="text-xs text-black">
                      All Status
                    </SelectItem>
                    <SelectItem value="active" className="text-xs text-black">
                      Active
                    </SelectItem>
                    <SelectItem value="prospect" className="text-xs text-black">
                      Prospect
                    </SelectItem>
                    <SelectItem value="inactive" className="text-xs text-black">
                      Inactive
                    </SelectItem>
                  </SelectContent>
                </Select>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-gray-50">
                      <th className="text-left py-2.5 px-3 text-gray-600 font-medium">
                        Company
                      </th>
                      <th className="text-left py-2.5 px-3 text-gray-600 font-medium">
                        Contact
          ...
          <div className="relative z-10 w-full max-w-md mx-4">
            <div className="bg-white border border-gray-200 rounded p-5">
              ...
              <div className="space-y-3">
                ...
                <Select
                  value={String(dealClientId ?? "")}
                  onValueChange={v => setDealClientId(Number(v))}
                >
                  <SelectTrigger className="h-8 bg-white border-gray-200 text-black text-xs">
            ...
            <div className="bg-white border border-gray-200 rounded p-5">
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <SelectContent className="bg-white border-gray-200">
                    <SelectItem value="low" className="text-xs text-black">
                      Low
                    </SelectItem>
                    <SelectItem value="medium" className="text-xs text-black">
                      Medium
                    </SelectItem>
                    <SelectItem value="high" className="text-xs text-black">
                      High
                    </SelectItem>
                  </SelectContent>
                </Select>
                <Select
                  value={taskClientId === null ? "none" : String(taskClientId)}
                  onValueChange={v =>
                    setTaskClientId(v === "none" ? null : Number(v))
                  }
                >
                  <SelectTrigger className="h-8 bg-white border-gray-200 text-black text-xs">
            ...
            <div className="bg-white border border-gray-200 rounded p-5">
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <Input
                    placeholder="Unit ID (e.g. GPS-001)"
                    value={eqUnitId}
                    onChange={e => setEqUnitId(e.target.value)}
                    className="bg-white border-gray-200 text-black"
                  />
                  <Input
                    placeholder="Type (e.g. GPS Tracker)"
                    value={eqType}
                ...
                <Input
                  ...
                  type="number"
                  value={eqHours}
                  onChange={e => setEqHours(e.target.value)}
                  className="bg-white border-gray-200 text-black"
                />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 w-full max-w-md mx-4">
            <div className="bg-white border border-gray-200 rounded p-5">
              ...
              <div className="space-y-3">
                <Select
                  value={String(editDealClientId ?? "")}
                  onValueChange={v => setEditDealClientId(Number(v))}
                >
                  <SelectTrigger className="h-8 bg-white border-gray-200 text-black text-xs">
                    <SelectValue placeholder="Select client" />
                  </SelectTrigger>
            ...
            <div className="bg-white border border-gray-200 rounded p-5">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  className="flex-1 bg-[#EF4444] hover:bg-[#EF4444]/80"
                >
                  Delete
                </Button>
              </div>
...
function ClientProfile({
  ...
  const [profileTab, setProfileTab] = useState<
    "overview" | "assets" | "billing" | "history"
  >("overview");
              ...
              <div className="data-card p-5 space-y-4">
                ...
                <div className="space-y-2">
                  {data.deals.map((deal: Deal) => (
                    <div
                      key={deal.id}
                      className="flex items-center justify-between p-3 rounded bg-white border border-gray-200"
                    >
                      <div>
                        <div className="text-xs font-bold text-black">
                          {deal.title}
                      ...
                      <div className="text-right">
                        <div className="text-xs font-bold text-[#66B2B2] font-mono-tech">
            ...
            <div className="col-span-4 space-y-6">
              ...
              <div className="data-card p-5 space-y-4">
                ...
                {data.bookings.map((booking: Booking) => (
                  <div
                    key={booking.id}
                    className="p-2.5 rounded bg-gray-50 border border-gray-200"
                  >
                ...
                <tbody>
                  {data.equipment.map((eq: Equipment) => (
                    ...
                    >
                      ...
                      <td className="py-2.5 px-3 text-black font-mono-tech">
                        {eq.currentHours}h
                      </td>
                      <td className="py-2.5 px-3">
          ...
          <div className="col-span-12 space-y-6">
            <div className="grid grid-cols-3 gap-6">
              {data.packages.map((pkg: Package) => (
                <div
                  key={pkg.id}
                  className="data-card p-5 border-t-4 border-t-[#66B2B2]"
                >
                  <div className="text-[10px] text-[#66B2B2] font-bold uppercase mb-1">
                    {pkg.tier} Package
                  </div>
                  <div className="text-lg font-bold text-black">{pkg.name}</div>
                  <div className="text-2xl font-bold text-black mt-4 font-mono-tech">
                    ₱{pkg.price}/mo
                  </div>
            ...
            <div className="data-card overflow-auto">
              <h3 className="p-4 text-sm font-bold text-black">
                Recent Invoices
              </h3>
              <table className="w-full text-xs">
                <thead>
                ...
                <tbody>
                  {data.invoices.map((inv: Invoice) => (
                    <tr key={inv.id} className="border-b border-gray-200">
                      ...
                      <td className="py-2.5 px-3 text-[#66B2B2] font-bold font-mono-tech">
                        ₱{inv.total.toFixed(2)}
                      </td>
                      <td className="py-2.5 px-3">
                ...
                <tbody>
                  {data.serviceHistory.map((s: ServiceRecord) => (
                    <tr key={s.id} className="border-b border-gray-200">
                      ...
                      <td className="py-2.5 px-3 text-black font-bold">
                        {data.equipment.find(
                          (e: Equipment) => e.id === s.equipmentId
                        )?.unitId || "—"}
                      </td>
                      <td className="py-2.5 px-3">
                        <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-[#66B2B2]/10 text-[#66B2B2] uppercase">
...
function StatusBadge({ status }: { status: Equipment["status"] }) {
  const configs: Record<Equipment["status"], { label: string; color: string }> =
    {
      ...
      maintenance: {
        label: "Maintenance",
        color: "bg-[#66B2B2]/10 text-[#66B2B2]",
      },
      ...
      service_due: {
        label: "Service Due",
        color: "bg-[#66B2B2]/20 text-[#66B2B2] animate-pulse",
      },
    };
  ...
  return (
    <span
      className={`px-1.5 py-0.5 rounded text-[9px] font-bold uppercase ${config.color}`}
    >
      {config.label}
    </span>
  );
          ...
          <div className="relative bg-white border border-gray-200 rounded-lg p-6 max-w-sm w-full animate-in zoom-in-95 duration-200">
            ...
            <div className="text-center space-y-4">
              ...
              <p className="text-[10px] text-gray-600 leading-relaxed">
                Scan this code to view service history or log a new maintenance
                record.
              </p>
...
function ClientRow({
  ...
  return (
    ...
    >
      ...
      <td className="py-2.5 px-3">
        <span
          className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${statusColors[client.status]}`}
        >
          {client.status}
        </span>
      </td>
  ...
  return (
    <div className="data-card p-3">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <span
            className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${priorityColors[lead.priority]}`}
          >
            {lead.priority}
          </span>
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] text-[#8B5CF6] font-mono-tech font-bold">
          Score: {lead.score}
        </span>
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div
          className={`text-xs font-medium ${overdue ? "text-[#EF4444]" : "text-black"}`}
        >
          {task.title}
        </div>
...
function SalesPerformance({
  ...
  const stats = salespeople.map(person => {
    ...
    const contactedLeads = leads.filter(
      l => l.assignedTo === person && l.status !== "new"
    ).length;
    const wonDeals = deals.filter(
      d => d.assignedTo === person && d.stage === "closed_won"
    ).length;
    const overdueTasks = tasks.filter(
      t => t.assignedTo === person && t.status === "overdue"
    ).length;
    const conversion =
      assignedLeads > 0 ? Math.round((wonDeals / assignedLeads) * 100) : 0;
    ...
    <div className="space-y-4 animate-in fade-in duration-500">
      <div className="grid grid-cols-4 gap-3">
        <div className="data-card p-4">
          ...
          <div className="text-xl font-bold text-[#10B981]">
            {stats.sort((a, b) => b.won - a.won)[0].name}
          </div>
          <div className="text-[10px] text-gray-600 mt-1">
            Most deals closed
          </div>
        ...
        <div className="data-card p-4">
          ...
          <div className="text-xl font-bold text-[#66B2B2]">
            {Math.round(
              stats.reduce((s, a) => s + a.conversion, 0) / stats.length
            )}
            %
          </div>
          <div className="text-[10px] text-gray-600 mt-1">
        ...
        <div className="data-card p-4 border border-[#EF4444]/20 bg-[#EF4444]/5">
          <div className="text-[10px] text-[#EF4444] uppercase font-bold mb-1">
            Critical Follow-ups
          </div>
      ...
      <div className="data-card overflow-hidden">
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-gray-50">
              <th className="text-left py-3 px-4 text-gray-600 font-medium uppercase tracking-wider text-[10px]">
                Salesperson
              </th>
              <th className="text-left py-3 px-4 text-gray-600 font-medium uppercase tracking-wider text-[10px]">
                Assigned Leads
          ...
          <tbody>
            {stats.map((s, i) => (
              ...
              >
                ...
                <td className="py-3 px-4 text-gray-600 font-mono-tech">
                  {s.assigned}
                </td>
                <td className="py-3 px-4 text-gray-600 font-mono-tech">
                  {s.contacted}
                </td>
                <td className="py-3 px-4 text-[#10B981] font-bold font-mono-tech">
                  {s.won}
                </td>
                <td className="py-3 px-4">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-[#66B2B2] w-8">
                      {s.conversion}%
                    </span>
                ...
                <td className="py-3 px-4">
                  <span
                    className={`font-bold ${s.overdue > 0 ? "text-[#EF4444]" : "text-gray-600"}`}
                  >
                    {s.overdue}
                  </span>
                </td>
