...
const CATEGORY_BADGE: Record<ServiceCategory, string> = {
  ...
  "Lab Testing Service":
    "bg-purple-50 text-[#7C3AED] border border-[#C084FC]/20",
...
function getHeavyRemaining(equipment: Equipment): number {
  const config = equipment.pmsConfiguration?.find(
    c => c.serviceIntervalUnit?.toLowerCase() === "hours"
  );
  if (!config) return Infinity;
  const current = parseHoursNum(equipment.hoursTotal);
  const next =
    (Math.floor(current / config.serviceInterval) + 1) * config.serviceInterval;
  return next - current;
}
...
function getCalibrationStatus(
  ...
  const diffDays = Math.ceil(
    (new Date(nextCalibrationDate).getTime() - now.getTime()) / 86400000
  );
...
export default function ClientDashboard() {
  ...
  const { equipment, serviceRecords, servicePhotos, bookings } =
    useOperationsStore();
  ...
  const clientEquipment = useMemo(
    () => equipment.filter(entry => eqClientNum(entry.clientId) === clientId),
    [equipment, clientId]
  );
  const clientEquipmentIds = useMemo(
    () => new Set(clientEquipment.map(e => e.id)),
    [clientEquipment]
  );
  const clientServices = useMemo(
    () =>
      serviceRecords.filter(entry => clientEquipmentIds.has(entry.equipmentId)),
    [serviceRecords, clientEquipmentIds]
  );
  const clientBookings = useMemo(
    () => bookings.filter(entry => eqClientNum(entry.clientId) === clientId),
    [bookings, clientId]
  );
  const clientInvoices = useMemo(
    () => invoices.filter(entry => entry.clientId === clientId),
    [invoices, clientId]
  );

  const equipmentById = useMemo(
    () => new Map(clientEquipment.map(entry => [entry.id, entry])),
    [clientEquipment]
  );
  ...
  const heavyEquipment = clientEquipment.filter(
    entry => entry.equipmentType === "Heavy Equipment"
  );
  const calibrationEquipment = clientEquipment.filter(
    entry =>
      entry.equipmentType !== "Heavy Equipment" && !!entry.nextCalibrationDate
  );
  const labTestingServices = clientServices.filter(
    entry => entry.serviceCategory === "Lab Testing Service"
  );
  ...
  const heavyDonutData: DonutDatum[] = [
    ...
    {
      name: "Near Service",
      value: heavyCounts["Near Service"],
      color: HEAVY_COLORS["Near Service"],
    },
    {
      name: "Overdue",
      value: heavyCounts.Overdue,
      color: HEAVY_COLORS.Overdue,
    },
    {
      name: "Due Soon",
      value: heavyCounts["Due Soon"],
      color: HEAVY_COLORS["Due Soon"],
    },
  ...
  const calibrationDonutData: DonutDatum[] = [
    ...
    {
      name: "Due Soon",
      value: calibrationCounts["Due Soon"],
      color: CALIBRATION_COLORS["Due Soon"],
    },
    {
      name: "Overdue",
      value: calibrationCounts.Overdue,
      color: CALIBRATION_COLORS.Overdue,
    },
    {
      name: "Due This Month",
      value: calibrationCounts["Due This Month"],
      color: CALIBRATION_COLORS["Due This Month"],
    },
  ].filter(entry => entry.value > 0);
  ...
  const labDonutData: DonutDatum[] = [
    {
      name: "In Progress",
      value: labCounts["In Progress"],
      color: LAB_COLORS["In Progress"],
    },
    {
      name: "Scheduled",
      value: labCounts.Scheduled,
      color: LAB_COLORS.Scheduled,
    },
    {
      name: "Completed",
      value: labCounts.Completed,
      color: LAB_COLORS.Completed,
    },
  ].filter(entry => entry.value > 0);
  ...
  const upcomingRows = useMemo<OverviewRow[]>(() => {
    return clientBookings
      ...
      .map(entry => {
        ...
        const heavyStatus =
          linkedEquipment?.equipmentType === "Heavy Equipment"
            ? getHeavyStatus(linkedEquipment)
            : null;
        ...
        return {
          ...
          scheduleLabel:
            linkedEquipment?.equipmentType === "Heavy Equipment" &&
            linkedEquipment.pmsConfiguration?.length
  ...
  const calibrationRows = useMemo<OverviewRow[]>(() => {
    return calibrationEquipment
      ...
      .slice(0, 4)
      .map(entry => {
        const daysUntil = entry.nextCalibrationDate
          ? getDaysUntil(entry.nextCalibrationDate, now)
          : 0;
        const status = getCalibrationStatus(entry.nextCalibrationDate, now);
        ...
        return {
          ...
          scheduleMeta: entry.nextCalibrationDate
            ? formatShortDate(entry.nextCalibrationDate)
            : "No date set",
          statusLabel: status,
    ...
    return labTestingServices
      ...
      .map(entry => {
        ...
        return {
          ...
          equipmentMeta: entry.projectName
            ? `Project: ${entry.projectName}`
            : entry.sampleName
              ? `Sample: ${entry.sampleName}`
              : getEquipmentSubLabel(linkedEquipment),
          category: "Lab Testing Service",
          typeLabel: entry.testType || "Lab Test",
          scheduleLabel: formatShortDate(
            entry.completedDate || entry.scheduledDate
          ),
          scheduleMeta: entry.completedDate
            ? "Report available"
            : "Queue active",
          statusLabel: status,
  ...
  const alertItems = useMemo<AlertItem[]>(() => {
    ...
    const releasableReports = labTestingServices.filter(
      entry => entry.labStatus === "Released" || entry.labStatus === "Completed"
    );
  ...
  const quickSummary = useMemo(() => {
    const completedThisMonth = clientServices.filter(
      entry =>
        entry.completedDate && new Date(entry.completedDate) >= monthStart
    );

    return {
      servicesCompleted: completedThisMonth.filter(
        entry =>
          entry.serviceCategory !== "Calibration PMS" &&
          entry.serviceCategory !== "Lab Testing Service"
      ).length,
      calibrationsCompleted: completedThisMonth.filter(
        entry => entry.serviceCategory === "Calibration PMS"
      ).length,
      testsCompleted: completedThisMonth.filter(
        entry => entry.serviceCategory === "Lab Testing Service"
      ).length,
  ...
  const latestReports = useMemo(() => {
    return clientServices
      ...
      .sort(
        (left, right) =>
          new Date(right.completedDate || right.createdAt).getTime() -
          new Date(left.completedDate || left.createdAt).getTime()
      )
      .slice(0, 3)
      .map(entry => ({
        record: entry,
  ...
  const topHeavyRisk = heavyEquipment
    .slice()
    .sort(
      (left, right) => getHeavyRemaining(left) - getHeavyRemaining(right)
    )[0];
    ...
    <div className="min-h-full bg-gray-100 px-6 py-5 text-gray-900 xl:px-8">
      <div className="mx-auto max-w-[1500px] space-y-5">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <h1 className="text-[34px] font-bold tracking-[-0.03em] text-gray-900">
              Dashboard
            </h1>
        ...
        <div className="grid gap-4 xl:grid-cols-[1.1fr_1.1fr_1.1fr_1.25fr]">
          ...
          <div className="data-card rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-5 text-[34px] font-bold tracking-[-0.03em] text-[#66B2B2]">
          ...
          <div className="space-y-4">
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-3 py-3">
                            <div className="flex items-center gap-3">
                              <div
                                className={`flex h-11 w-11 items-center justify-center rounded-lg border border-gray-200 ${row.iconBg}`}
                              >
                          ...
                          <td className="px-3 py-3">
                            <span
                              className={`inline-flex rounded-full px-2.5 py-1 text-[11px] font-medium ${CATEGORY_BADGE[row.category]}`}
                            >
                              {row.category}
                            </span>
                          </td>
                          ...
                          <td className="px-3 py-3">
                            <div
                              className={`text-[13px] font-medium ${row.statusLabel === "Overdue" ? "text-[#DC2626]" : row.statusLabel === "Due Soon" || row.statusLabel === "Near Service" ? "text-[#D97706]" : "text-[#059669]"}`}
                            >
                              {row.scheduleLabel}
                            </div>
                            <div className="mt-1 text-[11px] text-gray-500">
                          ...
                          <td className="px-3 py-3">
                            <span
                              className={`inline-flex rounded-full px-2.5 py-1 text-[11px] font-medium ${STATUS_BADGE[row.statusLabel] ?? STATUS_BADGE.Scheduled}`}
                            >
                              {row.statusLabel}
                            </span>
                          </td>
            ...
            <div className="grid gap-4 xl:grid-cols-3">
              <OverviewCard
                ...
                footerTitle={
                  topHeavyRisk
                    ? `${getEquipmentLabel(topHeavyRisk)}`
                    : "No critical unit"
                }
                footerSubtitle={
                  topHeavyRisk
              ...
              <OverviewCard
                ...
                footerTitle={
                  topCalibrationRisk
                    ? `${getEquipmentLabel(topCalibrationRisk)}`
                    : "No calibration risk"
                }
                footerSubtitle={
              ...
              <OverviewCard
                ...
                footerTitle={
                  topLabItem ? topLabItem.equipmentName : "No active tests"
                }
                footerSubtitle={
                  topLabItem
                    ? topLabItem.equipmentMeta
                    : "Testing queue is clear"
                }
              />
            </div>
            ...
            <section className="data-card rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
              <div className="mb-3 flex items-center justify-between">
                ...
                <button
                  className="text-sm font-medium text-[#66B2B2]"
                  onClick={() => navigate("/client/history")}
                >
                  View all
                </button>
              ...
              <div className="flex gap-2.5 overflow-x-auto pb-0.5">
                {recentActivity.map(item => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full shadow-sm border border-gray-100 ${item.tone}`}
                      >
                        <Icon className="h-4 w-4" />
                      </div>
                      <div className="min-w-0">
                        <div className="text-[11px] font-medium leading-4 text-gray-900">
                          {item.title}
                        </div>
          ...
          <div className="space-y-4">
            <section className="data-card rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                ...
                <button className="text-sm font-medium text-[#66B2B2]">
                  View all
                </button>
              </div>
            ...
            <section className="data-card rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                ...
                <span className="rounded-md border border-gray-200 bg-gray-100 px-3 py-1.5 text-xs text-gray-500">
                  This Month
                </span>
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                <QuickSummaryCard
                  ...
                  value={quickSummary.servicesCompleted}
                  icon={Wrench}
                  tint="bg-emerald-50 text-[#059669]"
                />
                <QuickSummaryCard
                  label="Calibrations Completed"
                  value={quickSummary.calibrationsCompleted}
                  icon={PackageCheck}
                  tint="bg-emerald-50 text-[#059669]"
                />
                <QuickSummaryCard
                  label="Tests Completed"
                  value={quickSummary.testsCompleted}
                  icon={ClipboardList}
                  tint="bg-purple-50 text-[#7C3AED]"
                />
                <QuickSummaryCard
                  label="Total Spent"
                  value={formatPeso(quickSummary.totalSpent)}
                  icon={Receipt}
                  tint="bg-blue-50 text-[#66B2B2]"
                />
              </div>
            </section>
            ...
            <section className="data-card rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
              ...
              <div className="space-y-3">
                {latestReports.map(report => (
                  <div
                    key={report.id}
                    className="flex items-center gap-3 rounded-xl border border-gray-200 bg-white p-3 hover:border-[#66B2B2]/40 transition-colors cursor-pointer group"
                    onClick={() => setShowReport(report.record)}
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-sm font-medium text-gray-900 group-hover:text-[#66B2B2] transition-colors">
                        {report.title}
                      </div>
                      <div className="mt-1 text-xs text-gray-500">
                        {report.date}
...
function SummaryCard({
  ...
  return (
    <div
      className={`data-card rounded-xl border border-gray-200 bg-gradient-to-br ${tint} p-5 shadow-sm`}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <div className="mt-1 flex items-end gap-2">
            <span className="text-[36px] font-bold leading-none tracking-[-0.04em] text-gray-900">
              {value}
            </span>
      ...
      <div
        className={`mt-5 grid gap-3 text-sm ${stats.length === 3 ? "grid-cols-3" : "grid-cols-4"}`}
      >
        {stats.map(stat => (
          <div key={stat.label}>
    ...
    <div className="rounded-xl border border-gray-200 bg-white p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="mt-2 text-[24px] font-semibold tracking-[-0.03em] text-gray-900">
            {value}
          </div>
        </div>
        <div
          className={`flex h-10 w-10 items-center justify-center rounded-lg bg-white shadow-sm border border-gray-100 ${tint.split(" ")[1]}`}
        >
          <Icon className="h-4 w-4" />
        </div>
      </div>
  ...
  return (
    <div className="p-2 animate-in fade-in zoom-in-95 duration-300">
      <div className="flex items-center justify-between border-b-2 border-gray-900 pb-6 mb-8">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-black text-white rounded-full text-[9px] font-black uppercase tracking-[0.2em] mb-3">
            <div className="w-2 h-2 rounded-full bg-[#66B2B2] animate-pulse" />{" "}
            Official Document
          </div>
          <h2 className="text-3xl font-black text-gray-900 tracking-tighter">
            TECHNICAL SERVICE REPORT
          </h2>
          <p className="text-xs text-gray-400 font-bold tracking-widest mt-1 font-mono-tech">
            NEXVISION OPS SYSTEM RECORD{" "}
            <span className="text-gray-900 font-black">#SR-{record.id}</span>
          </p>
        </div>
        <Button
          onClick={() => window.print()}
          className="bg-gray-100 hover:bg-gray-200 text-gray-900 h-12 px-6 border border-gray-200 text-xs font-black rounded-xl transition-all active:scale-95 shadow-sm"
        >
          ...
          <section>
            ...
            <div className="space-y-3 px-3">
              <div className="flex justify-between border-b border-gray-100 pb-2">
                ...
                <span className="text-[11px] text-gray-900 font-black font-mono-tech tracking-wider">
                  {equipment?.id}
                </span>
              </div>
              <div className="flex justify-between border-b border-gray-100 pb-2">
              ...
              <div className="flex justify-between border-b border-gray-100 pb-2">
                ...
                <span className="text-sm text-[#66B2B2] font-black font-mono-tech">
                  {record.hoursAtService} HOURS
                </span>
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="space-y-3 px-3">
              <div className="flex justify-between border-b border-gray-100 pb-2">
                ...
                <span className="text-[11px] text-gray-900 font-black uppercase tracking-tighter">
        ...
        <div className="space-y-8">
          <section className="h-full">
            ...
            <div className="p-6 rounded-2xl bg-gray-50 border border-gray-100 space-y-6 shadow-inner h-[calc(100%-2.5rem)]">
              <div>
                <span className="text-[10px] text-[#66B2B2] font-black uppercase tracking-widest mb-2 block">
                  Fault Diagnosis / Findings:
                </span>
                <p className="text-[12px] text-gray-900 font-medium leading-relaxed bg-white p-3 rounded-lg border border-gray-100">
                  {record.findings ||
                    "Operational state nominal. No significant faults detected during primary inspection."}
                </p>
              </div>
              <div>
                <span className="text-[10px] text-[#66B2B2] font-black uppercase tracking-widest mb-2 block">
              ...
              <div>
                ...
                <p className="text-[12px] text-gray-900 font-bold leading-relaxed italic bg-[#66B2B2]/5 p-3 rounded-lg border border-[#66B2B2]/10">
                  {record.recommendation ||
                    "No immediate action required. Maintain standard PMS intervals."}
                </p>
              </div>
            </div>
          </section>
        ...
        <div className="grid grid-cols-2 gap-10">
          <div className="space-y-3">
            <div className="flex items-center justify-between px-2">
              ...
              <span className="text-[8px] font-bold text-[#66B2B2] uppercase">
                Before Service
              </span>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {photos
                ...
                .map((p: any, i: number) => (
                  <div
                    key={i}
                    className="aspect-video rounded-xl overflow-hidden border-2 border-gray-100 shadow-md"
                  >
                    <img
                      src={p.url}
              ...
              {photos.filter((p: any) => p.type === "before").length === 0 && (
                <div className="col-span-2 py-8 text-center bg-gray-50 rounded-xl border border-dashed text-[10px] text-gray-400 font-bold uppercase tracking-widest">
                  No Before Documentation
                </div>
              )}
            </div>
          </div>
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {photos.filter((p: any) => p.type === "after").length === 0 && (
                <div className="col-span-2 py-8 text-center bg-gray-50 rounded-xl border border-dashed text-[10px] text-gray-400 font-bold uppercase tracking-widest">
                  No After Documentation
                </div>
              )}
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 gap-12 border-t-2 border-gray-100 pt-10 mb-6">
        <div className="space-y-4">
          <span className="text-[10px] text-gray-400 font-black uppercase tracking-[0.3em] block ml-1">
          ...
          <div className="p-8 bg-gray-50/50 rounded-3xl border border-gray-100 flex items-center justify-center shadow-inner relative overflow-hidden">
            {record.techSignature ? (
              <img
                ...
                className="h-24 object-contain contrast-125 mix-blend-multiply transition-all hover:scale-105 duration-500"
                alt="Tech Sig"
              />
            ) : (
              <div className="h-24 flex items-center justify-center text-gray-300 italic text-[11px] font-bold uppercase tracking-widest">
                Digital Stamp Missing
              </div>
            )}
            <div className="absolute bottom-4 left-0 right-0 text-center">
              ...
              <span className="text-[9px] text-gray-500 font-black uppercase tracking-tighter">
                {record.technician}{" "}
                <span className="text-gray-300 mx-1">•</span> SENIOR TECHNICIAN
              </span>
            </div>
          </div>
        </div>
        <div className="space-y-4">
          ...
          <div className="p-8 bg-gray-50/50 rounded-3xl border border-gray-100 flex items-center justify-center shadow-inner relative overflow-hidden">
            ...
            <div className="absolute bottom-4 left-0 right-0 text-center">
              ...
              <span className="text-[9px] text-gray-500 font-black uppercase tracking-tighter">
                {client?.companyName}{" "}
                <span className="text-gray-300 mx-1">•</span> AUTHORIZED REP
              </span>
            </div>
          </div>
        </div>
