...
function getPackageOption(booking: Booking): PackageOption {
  ...
  if (meta && Object.prototype.hasOwnProperty.call(PACKAGE_LABELS, meta))
    return meta;
...
export default function ClientBookings() {
  ...
  const { bookings, equipment, addBooking, updateBooking } =
    useOperationsStore();
  ...
  const clientEquipment = useMemo(
    () => equipment.filter(e => eqClientNum(e.clientId) === clientId),
    [equipment, clientId]
  );
  const equipmentById = useMemo(
    () => new Map(clientEquipment.map(eq => [eq.id, eq])),
    [clientEquipment]
  );
  ...
  const clientBookings = useMemo(
    () =>
      bookings
        ...
        .sort(
          (a, b) =>
            getBookingDateWithStartTime(a).getTime() -
            getBookingDateWithStartTime(b).getTime()
        ),
    [bookings, clientId]
  );
  ...
  const upcoming = useMemo(
    () => clientBookings.filter(b => !isBookingPast(b)),
    [clientBookings]
  );
  const past = useMemo(
    () => clientBookings.filter(b => isBookingPast(b)),
  ...
  const [openActionMenuFor, setOpenActionMenuFor] = useState<number | null>(
    null
  );
  ...
  const [bookingPackage, setBookingPackage] =
    useState<PackageOption>("pms_1000");
  ...
  const openReschedule = (booking: Booking) => {
    ...
    const tech =
      getTag(booking.notes, "tech") ??
      getTechnicianFromEquipmentId(booking.equipmentId);
  ...
  const filteredBookings = useMemo(
    () =>
      enrichedBookings.filter(item => {
        ...
        if (
          equipmentFilter !== "all" &&
          String(item.booking.equipmentId) !== equipmentFilter
        )
          return false;
        if (
          serviceTypeFilter !== "all" &&
          item.booking.serviceType !== serviceTypeFilter
        )
          return false;
        if (fromDate && item.booking.requestedDate.slice(0, 10) < fromDate)
          return false;
        if (toDate && item.booking.requestedDate.slice(0, 10) > toDate)
          return false;
  ...
  const completedCount = enrichedBookings.filter(
    b => b.status === "completed"
  ).length;
  const overdueCount = enrichedBookings.filter(
    b => b.isPast && b.status !== "completed" && b.status !== "cancelled"
  ).length;
  ...
  const statusBreakdown = STATUS_ORDER.reduce<
    Record<ExtendedBookingStatus, number>
  >(
    (acc, status) => {
  ...
  > = {
    completed: {
      ...
      color: "#10B981",
      getMessage: (id, t) => `Booking ${id} for ${t} has been completed.`,
    },
    confirmed: {
      icon: <Calendar className="w-4 h-4" />,
      bg: "rgba(59,130,246,0.15)",
      color: "#3B82F6",
      getMessage: (id, t) => `New booking ${id} for ${t} confirmed.`,
    },
    scheduled: {
      icon: <Clock className="w-4 h-4" />,
      bg: "rgba(139,92,246,0.15)",
      color: "#8B5CF6",
      getMessage: (id, t) => `Booking ${id} for ${t} has been scheduled.`,
    },
    in_progress: {
      icon: <Play className="w-4 h-4" />,
      bg: "rgba(245,158,11,0.15)",
      color: "#F59E0B",
      getMessage: (id, t) => `Booking ${id} for ${t} is now in progress.`,
    },
    pending: {
      icon: <Clock className="w-4 h-4" />,
      bg: "rgba(242,169,0,0.15)",
      color: "#66B2B2",
      getMessage: (id, t) => `Booking ${id} for ${t} was submitted.`,
    },
    cancelled: {
      icon: <XCircle className="w-4 h-4" />,
      bg: "rgba(239,68,68,0.15)",
      color: "#EF4444",
      getMessage: id => `Booking ${id} was cancelled.`,
    },
    rescheduled: {
      icon: <RotateCcw className="w-4 h-4" />,
      bg: "rgba(6,182,212,0.15)",
      color: "#06B6D4",
      getMessage: (id, t) => `Booking ${id} for ${t} was rescheduled.`,
    },
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            View, manage and track all your service bookings
          </p>
        </div>
        <Button
                  ...
                  <div className="flex items-center gap-2 mb-5">
                    {[0, 1, 2].map(step => (
                      ...
                      >
                        ...
                        {step < 2 && (
                          <div
                            className={`flex-1 h-0.5 ${bookingStep > step ? "bg-[#66B2B2]" : "bg-[#E5E7EB]"}`}
                          />
                        )}
                      </div>
                    ))}
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <Select
                          ...
                          onValueChange={setBookingEquipment}
                        >
                      ...
                      <div>
                        ...
                        <Select
                          ...
                          onValueChange={v =>
                            setBookingPackage(v as PackageOption)
                          }
                        >
                          <SelectTrigger className="h-9 bg-white border-gray-200 text-gray-900 text-xs">
                            <SelectValue />
                          ...
                          <SelectContent className="bg-white border-gray-200">
                            <SelectItem
                              ...
                              className="text-xs text-gray-900"
                            >
                              PMS Package (1000 hrs)
                            </SelectItem>
                            <SelectItem
                              value="repair_6"
                              className="text-xs text-gray-900"
                            >
                              Repair Package (6 visits)
                            </SelectItem>
                            <SelectItem
                              value="one_time"
                              className="text-xs text-gray-900"
                  ...
                  {bookingStep === 1 && (
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <Select
                          value={bookingTechnician}
                          onValueChange={setBookingTechnician}
                        >
                          <SelectTrigger className="h-9 bg-white border-gray-200 text-gray-900 text-xs">
                            <SelectValue />
                          </SelectTrigger>
                      ...
                      <div className="flex gap-2">
                        <Button
                          ...
                          className="flex-1 h-9 border-gray-200 text-gray-500 text-xs"
                        >
                          Back
                        </Button>
                        <Button
                      ...
                      <div className="p-3 rounded bg-gray-50 border border-gray-200 space-y-1 text-xs">
                        <div className="flex justify-between">
                          ...
                          <span className="text-gray-900 font-mono">
                            {clientEquipment.find(
                              e => e.id === bookingEquipment
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="text-gray-900">
                            {formatServiceType(bookingType)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-500">Package</span>
                          <span className="text-gray-900">
                            {PACKAGE_LABELS[bookingPackage]}
                          </span>
                        </div>
                        <div className="flex justify-between">
        ...
        <div className="xl:col-span-9 flex flex-col gap-3">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-3">
            ...
            <StatCard
              ...
              icon={<Clock className="w-4 h-4 text-[#66B2B2]" />}
              iconBg="bg-[#66B2B2]/20"
            />
            <StatCard
              title="Completed"
            ...
            <StatCard
              ...
              icon={<Ban className="w-4 h-4 text-[#EF4444]" />}
              iconBg="bg-[#EF4444]/20"
            />
            <StatCard
              title="Total Spent"
              value={formatMoneyPeso(24850)}
              subtitle="This month"
              icon={<DollarSign className="w-4 h-4 text-[#8B5CF6]" />}
              iconBg="bg-[#8B5CF6]/20"
            />
          </div>
              ...
              {(
                ...
                <button
                  ...
                  className={`text-xs pb-1 border-b transition-colors duration-150 ${
                    activeTab === key
                      ? "text-[#66B2B2] border-[#66B2B2]"
                      : "text-gray-500 border-transparent hover:text-gray-900"
                  }`}
                >
                  {label}{" "}
                  <span className="ml-1 text-[10px]">{tabCounts[key]}</span>
                </button>
              ))}
            </div>
              ...
              <table className="w-full min-w-[1100px] text-xs">
                <thead className="sticky top-0 z-10 bg-gray-50">
                  <tr className="border-b border-gray-200">
                    <th className="text-left py-2 px-2 text-[10px] uppercase tracking-wider text-gray-500">
                      Booking ID
                    </th>
                    <th className="text-left py-2 px-2 text-[10px] uppercase tracking-wider text-gray-500">
                      Equipment
                ...
                <tbody>
                  ...
                  {filteredBookings.map(item => {
                    ...
                    const serviceTypeKey =
                      typeof item.booking.serviceType === "string"
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-2 px-2">
                          <span
                            className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${SERVICE_COLORS[serviceTypeKey as ServiceType] ?? "bg-[#6B7280]/20 text-gray-500"}`}
                          >
                            {formatServiceType(item.booking.serviceType)}
                          </span>
                          <div className="text-[10px] text-gray-500 mt-1">
                            {item.booking.serviceType === "pms"
                              ? "(1000 hrs)"
                              : "One-time"}
                          </div>
                        </td>
                        ...
                        <td className="py-2 px-2">
                          <div className="flex items-center gap-1 text-gray-900">
                            ...
                            {formatDate(item.booking.requestedDate)}
                          </div>
                          <div className="flex items-center gap-1 text-[10px] text-gray-500 mt-1">
                            <Clock className="w-3 h-3" />
                            {getDisplayTime(item.booking.preferredTime)}
                          </div>
                        </td>
                        ...
                        <td className="py-2 px-2">
                          <div className="flex items-center gap-2">
                            <div
                              className={`w-6 h-6 rounded flex items-center justify-center text-[10px] font-semibold ${getTechColor(item.technician)}`}
                            >
                        ...
                        <td className="py-2 px-2">
                          <span
                            className={`px-1.5 py-0.5 rounded text-[10px] font-semibold capitalize ${STATUS_COLORS[item.status]}`}
                          >
                        ...
                        <td className="py-2 px-2">
                          <div className="flex items-center gap-1 text-gray-900">
                            ...
                            {PACKAGE_LABELS[item.packageType]}
                          </div>
                          <div className="text-[10px] text-gray-500 mt-1">
                            {item.packageType === "repair_6"
                              ? "(6 visits)"
                              : item.packageType === "pms_1000"
                                ? "(1000 hrs)"
                                : "One-time"}
                          </div>
                        </td>
                        ...
                        <td className="py-2 px-2">
                          <div
                            className={`font-semibold ${getHoursState(item.currentHours, item.nextPms)}`}
                          >
                            {item.currentHours.toLocaleString()} hrs
                          </div>
                          <div className="text-[10px] text-gray-500">
                            Next: {item.nextPms.toLocaleString()} hrs
                          </div>
                        </td>
                          ...
                          <div className="flex items-center gap-1 relative">
                            ...
                            {openActionMenuFor === item.booking.id && (
                              <div className="absolute right-0 top-8 w-40 rounded border border-gray-200 bg-gray-50 p-1 z-20">
                                <button className="w-full text-left px-2 py-1 text-xs text-gray-900 hover:bg-gray-50 rounded">
                                  View Details
                                </button>
                                <button
        ...
        <aside className="xl:col-span-3 flex flex-col gap-3">
          <div className="data-card p-3">
            ...
            <div className="grid grid-cols-7 gap-1 mt-1">
              {calendarDays.map((cell, index) => (
                <div
                  key={`day-${index}`}
                  className="h-8 rounded border border-gray-200 flex flex-col items-center justify-center text-[10px]"
                >
                  {cell.day && (
                    <>
                      <span
                        className={
                          cell.isToday
                            ? "text-white bg-[#66B2B2] px-1 rounded"
                            : "text-gray-900"
                        }
                      >
                        {cell.day}
                      ...
                      <span className="flex gap-0.5 mt-0.5">
                        {cell.hasUpcoming && (
                          <span className="w-1 h-1 rounded bg-[#3B82F6]" />
                        )}
                        {cell.hasOverdue && (
                          <span className="w-1 h-1 rounded bg-[#EF4444]" />
                        )}
                      </span>
                    </>
                  )}
            ...
            <div className="mt-2 flex items-center gap-3 text-[10px] text-gray-500">
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 rounded bg-[#66B2B2]" />
                Today
              </span>
              <span className="flex items-center gap-1">
          ...
          <div className="data-card p-3 flex flex-col flex-0.5">
            <div className="flex items-center justify-between">
              ...
              <button className="text-xs text-[#3B82F6] hover:text-gray-900 transition-colors duration-150">
                View all
              </button>
            </div>
              ...
              {miniUpcoming.map(item => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span
                      className={`px-1.5 py-0.5 rounded text-[10px] ${STATUS_COLORS[item.status]}`}
                    >
                      {item.status.replace("_", " ")}
                    </span>
                  </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-3">
              ...
              {[
                ...
                const pct =
                  enrichedBookings.length > 0
                    ? Math.round(
                        (Number(count) / enrichedBookings.length) * 100
                      )
                    : 0;
                return (
                  <div
                ...
                >
                  ...
                  <div>
                    <div className="text-xs text-gray-900">
                      {cfg.getMessage(bookingId, equipType)}
                    </div>
                    <div className="text-[10px] text-gray-500 mt-0.5">
                      {formatDate(item.bookingDate.toISOString())} ·{" "}
                      {getDisplayTime(item.booking.preferredTime)}
                    </div>
                  </div>
                </div>
        ...
        <div className="data-card p-3">
          ...
          <div className="mt-3 space-y-2">
            <button
              onClick={() => setBookingModalOpen(true)}
              className="w-full text-left p-2 rounded border border-gray-200 hover:bg-gray-50 transition-colors duration-150"
            >
              <div className="flex items-center gap-2">
                <PenSquare className="w-4 h-4 text-[#66B2B2]" />
                <span className="text-xs text-gray-900 font-semibold">
                  Book New Service
                </span>
              </div>
              <div className="text-[10px] text-gray-500 mt-1">
                Schedule a service for your equipment
              </div>
            </button>
            <button
              onClick={() => setBookingModalOpen(true)}
            ...
            >
              ...
              <div className="text-[10px] text-gray-500 mt-1">
                Need urgent assistance? Let us know
              </div>
            </button>
            <button className="w-full text-left p-2 rounded border border-gray-200 hover:bg-gray-50 transition-colors duration-150">
              ...
              <div className="text-[10px] text-gray-500 mt-1">
                Access your service reports and records
              </div>
            </button>
            <button className="w-full text-left p-2 rounded border border-gray-200 hover:bg-gray-50 transition-colors duration-150">
              <div className="flex items-center gap-2">
  ...
  return (
    <div className="data-card p-3">
      <div className="flex items-start justify-between">
        ...
        <div
          className={`w-8 h-8 rounded flex items-center justify-center ${iconBg}`}
        >
          {icon}
        </div>
      </div>
