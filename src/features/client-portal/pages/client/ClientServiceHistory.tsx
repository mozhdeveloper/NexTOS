...
export default function ClientServiceHistory() {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return serviceRecords
      ...
      .filter(record => {
        ...
        if (
          equipmentFilter !== "all" &&
          record.equipmentId !== equipmentFilter
        ) {
          return false;
        }
      ...
      .sort(
        (a, b) =>
          new Date(getServiceDate(b)).getTime() -
          new Date(getServiceDate(a)).getTime()
      );
  }, [
    clientId,
    dateFrom,
  ...
  const serviceTypes = useMemo(
    () =>
      Array.from(
        new Set(
          clientEquipment.flatMap(asset =>
            serviceRecords
              .filter(
                record =>
                  record.clientId === clientId &&
                  record.equipmentId === asset.id
              )
  ...
  const completedServices = allClientRecords.filter(
    record => record.status === "completed"
  ).length;
  const inProgressServices = allClientRecords.filter(
    record => record.status === "in_progress"
  ).length;
  const cancelledServices = allClientRecords.filter(
    record => record.status === "cancelled"
  ).length;
  const totalSpent = allClientRecords.reduce(
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {clientEquipment.length} equipment units · {filteredRecords.length}{" "}
        ...
        <div className="data-card p-4">
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="text-3xl font-bold text-gray-900 mt-1 font-mono-tech">
                {totalServices}
              </div>
        ...
        <div className="data-card p-4">
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="text-[11px] text-gray-500 mt-1">
                {totalServices > 0
                  ? `${Math.round((inProgressServices / totalServices) * 100)}%`
                  : "0%"}
              </div>
            </div>
            <div className="w-9 h-9 rounded-full bg-white shadow-sm flex items-center justify-center">
        ...
        <div className="data-card p-4">
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="text-[11px] text-gray-500 mt-1">
                {totalServices > 0
                  ? `${Math.round((cancelledServices / totalServices) * 100)}%`
                  : "0%"}
              </div>
            </div>
            <div className="w-9 h-9 rounded-full bg-white shadow-sm flex items-center justify-center">
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              ...
              {filteredRecords.map(record => {
                const asset = equipmentById.get(record.equipmentId);
                const photos = servicePhotos.filter(
                  photo => photo.serviceRecordId === record.id
                );
                ...
                const parseH = (t?: string) => {
                  const m = String(t ?? "").match(/(\d+)\s*h/i);
                  return m ? Number(m[1]) : 0;
                };
                const config = asset?.pmsConfiguration?.find(
                  (c: any) => c.serviceIntervalUnit?.toLowerCase() === "hours"
                );
                const currentH = parseH(asset?.hoursTotal);
                const nextMilestone = config
                  ? (Math.floor(currentH / config.serviceInterval) + 1) *
                    config.serviceInterval
                  : 0;
                const nextService = getNextServiceState(
                  currentH,
                  nextMilestone
                );
                  ...
                  <Fragment key={record.id}>
                    <tr className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <div className="text-base font-semibold text-gray-900 leading-tight">
                              {asset?.name ?? asset?.id ?? "Unknown Unit"}
                            </div>
                      ...
                      <td className="py-3 px-3">
                        ...
                        <div
                          className={`inline-flex mt-1 px-1.5 py-0.5 rounded text-[10px] font-medium ${serviceTypeColors[serviceTypeKey] ?? "bg-[#6B7280]/20 text-gray-500"}`}
                        >
                          {serviceTypeKey
                            ? serviceTypeKey.toUpperCase()
                            : "UNKNOWN"}
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-3">
                        <div className="text-gray-900 font-semibold">
                          {formatDate(getServiceDate(record))}
                        </div>
                        <div className="text-gray-400 mt-0.5">
                          {new Date(getServiceDate(record)).toLocaleTimeString(
                            undefined,
                      ...
                      <td className="py-3 px-3">
                        <div className="text-[#059669] font-semibold font-mono-tech">
                          {asset?.hoursTotal ?? "—"}
                        </div>
                      </td>

                      <td className="py-3 px-3">
                        <div className="text-gray-900 font-semibold font-mono-tech">
                          {config ? `${nextMilestone}h` : "—"}
                        </div>
                        <div
                          className={`mt-0.5 ${nextService.remainingHours <= 0 ? "text-[#EF4444]" : "text-[#059669]"}`}
                        >
                      ...
                      <td className="py-3 px-3">
                        <span
                          className={`inline-flex px-2 py-0.5 rounded text-[11px] font-medium capitalize ${statusColors[statusKey] ?? "bg-[#6B7280]/20 text-gray-500"}`}
                        >
                          {statusLabel}
                        </span>
                      </td>
                          ...
                          <div className="rounded-xl border border-[#66B2B2]/40 bg-white">
                            <div className="p-4 grid grid-cols-1 xl:grid-cols-[minmax(0,1.4fr)_repeat(5,minmax(0,0.8fr))_auto] gap-4 border-b border-gray-200">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <span
                                    className={`inline-flex mt-2 px-2 py-0.5 rounded text-[11px] font-medium capitalize ${statusColors[statusKey] ?? "bg-[#6B7280]/20 text-gray-500"}`}
                                  >
                                    {statusLabel}
                                  </span>
                                </div>
                              ...
                              <div>
                                ...
                                <div className="text-2xl text-gray-900 mt-0.5">
                                  {formatDate(getServiceDate(record))}
                                </div>
                                <div className="text-sm text-gray-400 mt-0.5">
                                  {new Date(
                              ...
                              <div>
                                ...
                                <div className="text-2xl text-[#059669] mt-0.5 font-mono-tech">
                                  {asset?.hoursTotal ?? "—"}
                                </div>
                              </div>
                              ...
                              <div>
                                ...
                                <div className="text-2xl text-gray-900 mt-0.5 font-mono-tech">
                                  {config ? `${nextMilestone}h` : "—"}
                                </div>
                            ...
                            <div className="p-4 grid grid-cols-1 lg:grid-cols-4 gap-4 text-xs">
                              <div className="space-y-2">
                                ...
                                <RowDetail
                                  label="Service Type"
                                  value={formatServiceType(record.serviceType)}
                                />
                                <RowDetail
                                  label="Service Category"
                                  value={
                                    record.serviceType === "pms"
                                ...
                                <RowDetail
                                  label="Work Order"
                                  value={`WO-${new Date(getServiceDate(record)).getFullYear()}-${String(record.id).padStart(4, "0")}`}
                                />
                                <RowDetail
                                  label="Location"
                              ...
                              <div>
                                ...
                                <div className="space-y-2">
                                  {workDoneItems.length > 0 ? (
                                    workDoneItems.map(item => (
                                      <div
                                        key={item}
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="mt-3 p-2 rounded-md bg-gray-50 border border-gray-200 flex items-center justify-between">
                                  ...
                                  <span
                                    className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${nextServiceStateColors[nextService.tone]}`}
                                  >
                                    {nextService.label}
                                  </span>
                                </div>
