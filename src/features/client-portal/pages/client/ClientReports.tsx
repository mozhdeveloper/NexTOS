...
export default function ClientReports() {
  ...
  const clientEquipment = equipment.filter(
    e => eqClientNum(e.clientId) === clientId
  );
  const clientEquipmentIds = new Set(clientEquipment.map(e => e.id));
  const clientServices = serviceRecords.filter(r =>
    clientEquipmentIds.has(r.equipmentId)
  );

  const clientBookings = bookings.filter(
    b => eqClientNum(b.clientId) === clientId
  );
  ...
  // High Value KPIs
  const completedServices = clientServices.filter(
  ...
  const getRemaining = (e: Equipment) => {
    const cfg = e.pmsConfiguration?.find(
      c => c.serviceIntervalUnit?.toLowerCase() === "hours"
    );
    if (!cfg) return null;
    const cur = parseH(e.hoursTotal);
    return (
      (Math.floor(cur / cfg.serviceInterval) + 1) * cfg.serviceInterval - cur
    );
  };
  ...
  const pmsReport = useMemo(() => {
    return clientEquipment
      .filter(e => e.equipmentType === "Heavy Equipment")
      .map(e => {
        const cfg = e.pmsConfiguration?.find(
          c => c.serviceIntervalUnit?.toLowerCase() === "hours"
        );
        const cur = parseH(e.hoursTotal);
        const next = cfg
          ? (Math.floor(cur / cfg.serviceInterval) + 1) * cfg.serviceInterval
          : 0;
  ...
  const calibrationReport = useMemo(() => {
    return clientEquipment
      .filter(
        e =>
          e.equipmentType === "Lab Equipment" ||
          e.equipmentType === "Testing Equipment"
      )
      .map(e => {
        return {
          unit: e.name ?? e.id,
  ...
  const testingReport = useMemo(() => {
    return clientServices
      .filter(s => s.serviceCategory === "Lab Testing Service")
      .map(s => {
        return {
          type: s.testType || "—",
    ...
    <div className="space-y-4 px-8 pt-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Fleet health, risk analysis, and service investment
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <MetricCard
          ...
          value={`${activeSubs}`}
          hint={`${clientPackages.length} total assigned`}
          icon={Package}
          accent="text-[#66B2B2]"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        <div className="data-card p-4">
          ...
          <p className="text-xs text-gray-500 mb-3">
            Health distribution of your fleet
          </p>
        ...
        <div className="data-card p-4">
          ...
          <p className="text-xs text-gray-500 mb-3">
            Services performed by category
          </p>
          <ResponsiveContainer width="100%" height={200}>
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {pmsReport.map((row, idx) => (
              ...
              >
                <td className="py-2.5 px-3 text-gray-900 font-mono-tech">
                  {row.unit}
                </td>
                ...
                <td
                  className={`py-2.5 px-3 font-mono-tech ${row.remaining <= 0 ? "text-[#EF4444]" : row.remaining <= 50 ? "text-[#66B2B2]" : "text-gray-500"}`}
                >
                  {row.remaining <= 0
                    ? `OVERDUE (${Math.abs(row.remaining)}h)`
                    : `${row.remaining}h left`}
                </td>
                <td className="py-2.5 px-3">
                  <span
                    className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${row.status === "OK" ? "bg-[#10B981]/20 text-[#10B981]" : row.status === "Near Service" ? "bg-[#66B2B2]/20 text-[#66B2B2]" : "bg-[#EF4444]/20 text-[#EF4444]"}`}
                  >
                    {row.status}
                  </span>
                </td>
      ...
      <div className="data-card overflow-auto">
        <div className="p-3 border-b border-gray-200 flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-gray-900">
            Package & Subscription Usage Report
          </h3>
        </div>
        <table className="w-full text-xs">
          <thead>
          ...
          <tbody>
            {clientPackages.map(pkg => (
              <tr key={pkg.id} className="border-b border-[#E5E7EB]">
                ...
                <td className="py-2.5 px-3 text-gray-900 font-mono-tech font-bold">
                  {pkg.visitsRemaining}
                </td>
                ...
                <td className="py-2.5 px-3">
                  <span
                    className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${pkg.status === "active" ? "bg-[#10B981]/20 text-[#10B981]" : "bg-[#6B7280]/20 text-gray-500"}`}
                  >
                    {pkg.status}
                  </span>
                </td>
            ...
            <tbody>
              {calibrationReport.map((row, idx) => (
                <tr key={idx} className="border-b border-[#E5E7EB]">
                  ...
                  <td
                    className={`py-2.5 px-3 font-mono-tech ${(row.remaining ?? 1) <= 0 ? "text-[#EF4444]" : "text-gray-500"}`}
                  >
                    {row.remaining ?? "—"}d
                  </td>
                  <td className="py-2.5 px-3">
                    <span
                      className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${row.status === "OK" ? "bg-[#10B981]/20 text-[#10B981]" : row.status === "Due Soon" ? "bg-[#66B2B2]/20 text-[#66B2B2]" : "bg-[#EF4444]/20 text-[#EF4444]"}`}
                    >
                      {row.status}
                    </span>
                  </td>
            ...
            <tbody>
              {testingReport.map((row, idx) => (
                <tr key={idx} className="border-b border-[#E5E7EB]">
                  ...
                  <td className="py-2.5 px-3">
                    <span
                      className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${row.status === "Released" ? "bg-[#10B981]/20 text-[#10B981]" : "bg-[#66B2B2]/20 text-[#66B2B2]"}`}
                    >
                      {row.status}
                    </span>
                  </td>
