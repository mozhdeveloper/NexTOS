...
export default function ClientBilling() {
  ...
  const upcomingDue = clientInvoices.filter(
    i =>
      i.status !== "paid" &&
  ...
  const invoiceBreakdown = clientInvoices.reduce(
    (acc, invoice) => {
  ...
  const breakdownData = Object.entries(invoiceBreakdown).map(
    ([name, value]) => ({
      name,
      value,
      color: name.includes("Package")
        ? "#66B2B2"
  ...
  const monthlySpending = clientInvoices.reduce(
    (acc, invoice) => {
  ...
  const spendingOverviewData = Object.entries(monthlySpending)
    .map(([month, amount]) => ({
      month,
      amount,
    }))
  ...
  // Service Summary
  const serviceSummary = clientInvoices.reduce(
    (acc, invoice) => {
      const category = getInvoiceCategory(invoice);
  ...
  const spendingCategoryData = Object.entries(invoiceBreakdown).map(
    ([name, value], index) => ({
      name,
      value,
    ...
    <div className="space-y-6 px-8 pt-8">
      ...
      <div className="grid gap-3 lg:grid-cols-[repeat(5,_minmax(0,1fr))]">
        ...
        <div className="data-card p-4 lg:col-span-1">
          ...
          <div className="text-3xl font-bold text-gray-900">
            ...
            {totalInvoices > 0
              ? (
                  clientInvoices.reduce((s, i) => s + i.total, 0) /
                  totalInvoices
                ).toFixed(2)
              : "0.00"}
          </div>
            ...
            <div className="flex flex-col gap-3 border-b border-gray-200 p-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="text-[10px] text-gray-500 mt-1">
                  Complete overview of all your invoices and payments
                </p>
              </div>
                ...
                <tbody>
                  {clientInvoices.map(invoice => (
                    ...
                    >
                      ...
                      <td className="py-4 px-3 align-top text-gray-500 font-mono-tech">
                        {getInvoiceAsset(invoice)}
                      </td>
                      <td className="py-4 px-3 align-top">
                        <div className="text-[#66B2B2] font-bold text-sm">
        ...
        <div className="space-y-4">
          ...
          <div className="data-card p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[10px] text-[#66B2B2]">
                ...
                upcomingDue.map(invoice => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="text-[10px] text-[#66B2B2] font-semibold">
                        ₱{invoice.total.toFixed(2)}
                      </div>
                    </div>
                    <div className="text-[10px] text-gray-500 mt-2">
          ...
          <div className="data-card p-4">
            ...
            <div className="space-y-3 text-[10px] text-gray-500">
              <div className="flex items-start gap-3">
                <div className="mt-1 h-6 w-6 rounded-full bg-[#66B2B2]/20 text-[#66B2B2] flex items-center justify-center">
                  1
                </div>
                <div>
                  ...
                  Invoices are generated when services are completed or packages
                  are purchased.
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  You'll receive an email notification with payment details and
                  due date.
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  Pay online securely or contact us for alternative payment
                  methods.
                </div>
              </div>
              <div className="flex items-start gap-3">
            ...
            <div className="space-y-3">
              {Object.entries(serviceSummary).map(([category, data]) => (
                ...
                >
                  ...
                  <div className="text-[#66B2B2] font-bold">
      ...
      <Dialog open={invoiceDetailsOpen} onOpenChange={setInvoiceDetailsOpen}>
        <DialogContent className="bg-gray-50 border border-gray-200 text-gray-900">
          <DialogHeader>
            <DialogTitle>
              Invoice Details - {selectedInvoiceDetails?.invoiceNumber}
            </DialogTitle>
          </DialogHeader>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-[10px] text-gray-500">
                    Created:{" "}
                    {new Date(
                      selectedInvoiceDetails.createdAt
                    ).toLocaleDateString()}
                  </p>
                  <p className="text-[10px] text-gray-500">
                    Due:{" "}
                ...
                <div>
                  ...
                  <p className="text-[10px] text-gray-500">
                    Subtotal: ₱
                    {selectedInvoiceDetails.subtotal?.toFixed(2) ||
                      selectedInvoiceDetails.total.toFixed(2)}
                  </p>
                  <p className="text-[10px] text-gray-500">
                    Tax: ₱{selectedInvoiceDetails.tax.toFixed(2)}
                  </p>
                  <p className="text-[#66B2B2] font-bold">
                    Total: ₱{selectedInvoiceDetails.total.toFixed(2)}
                  </p>
                </div>
              </div>
              <div>
                ...
                <p className="text-[10px] text-gray-500">
                  Category: {getInvoiceCategory(selectedInvoiceDetails)}
                </p>
                <p className="text-[10px] text-gray-500">
                  Asset: {getInvoiceAsset(selectedInvoiceDetails)}
                </p>
              </div>
            </div>
          )}
