...
export default function ClientBilling() {
  ...
  const chartData = [
    ...
    { name: "Outstanding", value: outstandingAmount, color: "#F2A900" },
  ];
  ...
  const breakdownData = Object.entries(invoiceBreakdown).map(([name, value]) => ({
    name,
    value,
    color: name.includes("Package") ? "#F2A900" : name.includes("Service") ? "#10B981" : "#88888C"
  }));
  ...
  const spendingCategoryData = Object.entries(invoiceBreakdown).map(([name, value], index) => ({
    name,
    value,
    color: ["#F2A900", "#10B981", "#EF4444", "#88888C"][index % 4]
  }));
  ...
  return (
    <div className="space-y-6 px-8 pt-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h1 className="text-[32px] font-bold text-[#EAEAEA] tracking-[-0.02em]">Billing & Invoices</h1>
          <p className="text-sm text-[#88888C] mt-1">Manage your invoices, payments and billing history</p>
        </div>
      ...
      <div className="grid gap-3 lg:grid-cols-[repeat(5,_minmax(0,1fr))]">
        ...
        <div className="data-card p-4 lg:col-span-1">
          ...
          <div className="text-3xl font-bold text-[#EAEAEA]">{totalInvoices}</div>
          <div className="text-[10px] text-[#88888C] mt-2">All time</div>
        </div>
        <div className="data-card p-4 lg:col-span-1">
          <div className="text-[10px] text-[#88888C] uppercase tracking-wider mb-3 flex items-center gap-2">
        ...
        <div className="data-card p-4 lg:col-span-1">
          ...
          <div className="text-3xl font-bold text-[#EAEAEA]">{upcomingDue.length}</div>
          <div className="text-[10px] text-[#88888C] mt-2">Within 7 days</div>
        </div>
        <div className="data-card p-4 lg:col-span-1">
          <div className="text-[10px] text-[#88888C] uppercase tracking-wider mb-3 flex items-center gap-2">
      ...
      <div className="grid gap-4 xl:grid-cols-[2fr_1fr]">
        <div className="space-y-4">
          <div className="data-card overflow-hidden">
            <div className="flex flex-col gap-3 border-b border-white/10 p-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="text-[10px] text-[#88888C] mt-1">Complete overview of all your invoices and payments</p>
              </div>
            ...
            <div className="overflow-auto">
              <table className="min-w-full text-left text-[11px]">
                <thead className="bg-[#0D111D]">
                  <tr>
                    <th className="py-3 px-3 text-[#88888C] font-medium">Invoice #</th>
                    <th className="py-3 px-3 text-[#88888C] font-medium">Service / Item</th>
                    <th className="py-3 px-3 text-[#88888C] font-medium">Asset</th>
                    <th className="py-3 px-3 text-[#88888C] font-medium">Total</th>
                    <th className="py-3 px-3 text-[#88888C] font-medium">Due Date</th>
                  ...
                  {clientInvoices.map((invoice) => (
                    <tr key={invoice.id} className="border-b border-white/10 hover:bg-white/5">
                      <td className="py-4 px-3 align-top">
                        <div className="font-semibold text-[#EAEAEA] text-sm">{invoice.invoiceNumber}</div>
                      ...
                      <td className="py-4 px-3 align-top text-[#EAEAEA]">
                        ...
                        <div className="text-[10px] text-[#88888C]">Created {new Date(invoice.createdAt).toLocaleDateString()}</div>
                      </td>
                      ...
                      <td className="py-4 px-3 align-top text-[#EAEAEA]">
                        ...
                        <div className="text-[10px] text-[#88888C]">Due in {Math.ceil((new Date(invoice.dueDate).getTime() - Date.now()) / (1000 * 60 * 60 * 24))} days</div>
                      </td>
                      <td className="py-4 px-3 align-top">
                        <span
                        ...
                        {invoice.status !== "paid" ? (
                          <div className="flex flex-col gap-1">
                            <Button
                              ...
                              className="bg-[#F2A900] text-[#050505] text-[10px] font-semibold px-3 py-2 hover:bg-[#D69E02]"
                            >
                            ...
                            <Button
                              ...
                              className="text-[10px] border-white/10 text-[#EAEAEA] hover:bg-white/5"
                            >
                              View Details
                            </Button>
                          </div>
          ...
          <div className="data-card p-4">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-[10px] text-[#88888C] mt-1">Spending by invoice category</p>
              </div>
              <PieChart className="w-4 h-4 text-[#88888C]" />
            </div>
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 space-y-2">
                {breakdownData.map((item) => (
                  <div key={item.name} className="flex items-center justify-between text-[10px] text-[#EAEAEA]">
          ...
          <div className="data-card p-4">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-sm font-semibold text-[#EAEAEA]">Spending Overview</h3>
                <p className="text-[10px] text-[#88888C] mt-1">Monthly spending trends</p>
              </div>
              <BarChart3 className="w-4 h-4 text-[#88888C]" />
            </div>
            <div className="h-32">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={spendingOverviewData}>
                  <Line type="monotone" dataKey="amount" stroke="#F2A900" strokeWidth={2} />
                  <XAxis dataKey="month" tick={{ fontSize: 10, fill: '#88888C' }} />
                  <YAxis tick={{ fontSize: 10, fill: '#88888C' }} />
                  <CartesianGrid stroke="#2A2A30" />
                  <Tooltip
                    contentStyle={{ backgroundColor: '#0D111D', border: '1px solid #2A2A30', borderRadius: '4px' }}
        ...
        <div className="space-y-4">
          <div className="data-card p-4">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-sm font-semibold text-[#EAEAEA]">Payment Summary</h3>
                <p className="text-[10px] text-[#88888C] mt-1">Paid vs outstanding amounts</p>
              </div>
              <div className="w-24 h-24">
                <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="data-card p-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-[#EAEAEA]">Upcoming Due Dates</h3>
              <span className="text-[10px] text-[#F2A900]">{upcomingDue.length} due</span>
            </div>
            <div className="space-y-3">
              {upcomingDue.length > 0 ? (
                upcomingDue.map((invoice) => (
                  <div key={invoice.id} className="rounded-xl border border-white/10 bg-[#0A0A0C] p-3">
                ...
                <div className="text-[10px] text-[#88888C]">No invoices due in the next 7 days.</div>
              )}
            </div>
          </div>
          ...
          <div className="data-card p-4">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-sm font-semibold text-[#EAEAEA]">Need Help with Billing?</h3>
                <p className="text-[10px] text-[#88888C] mt-1">Contact support for billing questions or disputes.</p>
              </div>
            </div>
          ...
          <div className="data-card p-4">
            <h3 className="text-sm font-semibold text-[#EAEAEA] mb-4">How Billing Works</h3>
            <div className="space-y-3 text-[10px] text-[#88888C]">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-semibold text-[#EAEAEA]">Services Completed</div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-semibold text-[#EAEAEA]">Invoice Sent</div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-semibold text-[#EAEAEA]">Payment Options</div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-semibold text-[#EAEAEA]">Confirmation</div>
          ...
          <div className="data-card p-4">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-sm font-semibold text-[#EAEAEA]">Service Summary</h3>
                <p className="text-[10px] text-[#88888C] mt-1">Overview of services performed</p>
              </div>
          ...
          <div className="data-card p-4">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-sm font-semibold text-[#EAEAEA]">Spending by Category</h3>
                <p className="text-[10px] text-[#88888C] mt-1">Breakdown of total spending</p>
              </div>
              <PieChart className="w-4 h-4 text-[#88888C]" />
            </div>
      ...
      <Dialog open={invoiceDetailsOpen} onOpenChange={setInvoiceDetailsOpen}>
        <DialogContent className="bg-[#0D111D] border border-white/10 text-[#EAEAEA]">
          <DialogHeader>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-[10px] text-[#88888C]">Created: {new Date(selectedInvoiceDetails.createdAt).toLocaleDateString()}</p>
                  <p className="text-[10px] text-[#88888C]">Due: {new Date(selectedInvoiceDetails.dueDate).toLocaleDateString()}</p>
                  <p className="text-[10px] text-[#88888C]">Status: {selectedInvoiceDetails.status}</p>
                </div>
                <div>
              ...
              <div>
                ...
                <p className="text-[10px] text-[#88888C]">Category: {getInvoiceCategory(selectedInvoiceDetails)}</p>
                <p className="text-[10px] text-[#88888C]">Asset: {getInvoiceAsset(selectedInvoiceDetails)}</p>
              </div>
            </div>
          )}
        </DialogContent>
