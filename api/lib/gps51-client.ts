...
const DEVICE_ID = process.env.GPS51_DEVICE_ID ?? "866557081650428";
...
const TODAY_SUMMARY_CACHE_TTL_MS = 5 * 60 * 1000;
...
function normalizeDurationMs(value: unknown, windowMs: number): number {
  const numeric = Number(value);
  if (!Number.isFinite(numeric) || numeric <= 0) {
    return 0;
  }

  // GPS51 sometimes returns duration values in seconds for a subset of fields.
  const msCandidate = numeric;
  const secCandidate = numeric * 1000;
  const secondsUpperBound = (windowMs / 1000) * 2;
  const looksLikeSeconds = Number.isInteger(numeric) && numeric <= secondsUpperBound;

  if (looksLikeSeconds && msCandidate < 5 * 60 * 1000 && secCandidate <= windowMs * 1.5) {
    return secCandidate;
  }
...
function getCachedDailySummaryEntry(day: string): CachedDailySummary | null {
  return dailySummaryCache.get(day) ?? null;
}

function shouldServeCachedDailySummary(day: string, entry: CachedDailySummary | null): boolean {
  if (!entry) {
    return false;
  }

  const isToday = day === toLocalYmd();
  if (!isToday) {
    return true;
  }

  return Date.now() - entry.fetchedAt < TODAY_SUMMARY_CACHE_TTL_MS;
}

function keepPrevText(nextValue: string | undefined, prevValue: string | undefined): string | undefined {
  const nextText = String(nextValue ?? "").trim();
  if (nextText) {
    return nextText;
  }
  const prevText = String(prevValue ?? "").trim();
  return prevText || undefined;
}

function pickStableNumber(nextValue: number | undefined, prevValue: number | undefined): number | undefined {
  ...
  return Number.isFinite(nextNum) ? nextNum : prevNum;
}

function stabilizeDailySummary(
  previous: GPS51DailyHistorySummary,
  current: GPS51DailyHistorySummary
): GPS51DailyHistorySummary {
  return {
    ...current,
    maxSpeedMps: Number(pickStableNumber(current.maxSpeedMps, previous.maxSpeedMps) ?? 0),
    avgSpeedMps: Number(pickStableNumber(current.avgSpeedMps, previous.avgSpeedMps) ?? 0),
    idleMs: Number(pickStableNumber(current.idleMs, previous.idleMs) ?? 0),
    startTime: pickStableNumber(current.startTime, previous.startTime),
    endTime: pickStableNumber(current.endTime, previous.endTime),
    startAddress: keepPrevText(current.startAddress, previous.startAddress),
    endAddress: keepPrevText(current.endAddress, previous.endAddress),
    startLat: pickStableNumber(current.startLat, previous.startLat),
    startLng: pickStableNumber(current.startLng, previous.startLng),
    endLat: pickStableNumber(current.endLat, previous.endLat),
    endLng: pickStableNumber(current.endLng, previous.endLng),
  };
}

function getCachedDailySummary(day: string): GPS51DailyHistorySummary | null {
  return dailySummaryCache.get(day)?.summary ?? null;
}

function setCachedDailySummary(day: string, summary: GPS51DailyHistorySummary): void {
  dailySummaryCache.set(day, {
    summary,
    fetchedAt: Date.now(),
  });
}

function resolveAddressFromMap(
  addressMap: Record<string, unknown> | null,
  lat: number | undefined,
  lng: number | undefined
): string {
  if (!addressMap || !Number.isFinite(lat) || !Number.isFinite(lng)) {
    return "";
  }

  const prefixes = [5, 4, 3];
  for (const digits of prefixes) {
    const key = `${Number(lat).toFixed(digits)}_${Number(lng).toFixed(digits)}`;
    const value = addressMap[key];
    if (typeof value === "string" && value.trim()) {
      return value.trim();
    }
  }

  // Fallback: pick nearest coordinate key from addressmap.
  let bestDistance = Number.MAX_VALUE;
  let bestAddress = "";
  for (const [key, value] of Object.entries(addressMap)) {
    if (typeof value !== "string" || !value.trim()) continue;
    const [latText, lngText] = key.split("_");
    const keyLat = Number(latText);
    const keyLng = Number(lngText);
    if (!Number.isFinite(keyLat) || !Number.isFinite(keyLng)) continue;
    const distance = Math.hypot(Number(lat) - keyLat, Number(lng) - keyLng);
    if (distance < bestDistance) {
      bestDistance = distance;
      bestAddress = value.trim();
    }
  }

  if (bestAddress && bestDistance <= 0.005) {
    return bestAddress;
  }
...
function pickTripTimestamp(
  ...
  // Keep timestamps scoped to the selected day. trackstart/trackend can span across days.
  const keys = mode === "start"
    ? ["starttime", "begintime", "trackstarttime"]
    : ["endtime", "stoptime", "trackendtime"];

  const values = trips
    .map((trip) => pickFirstNonNegative(...keys.map((key) => trip[key])))
  ...
  return mode === "start" ? Math.min(...values) : Math.max(...values);
}
    ...
    try {
      ...
      const response = await fetch(url, {
        headers: {
          ...
          "User-Agent": "NexTOS/1.0 (GPS51 history address resolver)",
          "Accept-Language": "en",
        },
      });
...
let loginPromise: Promise<GPS51Session> | null = null;
...
// Auto re-login if session expired — login lock prevents concurrent logins
async function getSession(username: string, password: string): Promise<GPS51Session> {
  ...
  if (!session || isExpired) {
    if (!loginPromise) {
      loginPromise = gps51Login(username, password).finally(() => {
        loginPromise = null;
      });
    }
    await loginPromise;
...
export async function fetchDailyHistorySummary(
  ...
  const cachedEntry = getCachedDailySummaryEntry(day);
  const cachedSummary = cachedEntry?.summary ?? null;
  if (shouldServeCachedDailySummary(day, cachedEntry)) {
    return cachedSummary;
  }
  const startTime = `${day} 00:00:00`;
  const endTime = `${day} 23:59:59`;

  // Pre-warm the session once before parallel calls to avoid concurrent login race
  ...
  const [tripsData, mileageData, tracksAdditionalData] = await Promise.all([
    ...
    // querytrackswithadditionals may be blocked by GPS51 IP tier — treat failure as empty
    postGps51Action("querytrackswithadditionals", username, password, {
      deviceid: DEVICE_ID,
      starttime: startTime,
      endtime: endTime,
      timezone: 8,
    }).catch(() => ({ status: -1, cause: "unavailable" })),
  ]);
  ...
  if (tripsData?.status !== 0 && mileageData?.status !== 0) {
    logGPS51("daily.summary.cache-fallback", {
      day,
      reason: "primary endpoints failed",
      tripsStatus: tripsData?.status,
      mileageStatus: mileageData?.status,
      hasCachedSummary: Boolean(cachedSummary),
    });
    return cachedSummary;
  }

  const trips = Array.isArray(tripsData?.totaltrips) ? tripsData.totaltrips : [];
  const tripAddressMap =
    tripsData && typeof tripsData.addressmap === "object" && tripsData.addressmap !== null
      ? tripsData.addressmap as Record<string, unknown>
      : null;
  ...
  const tripsMaxSpeed = trips.reduce(
    (max: number, trip: Record<string, unknown>) => Math.max(
      max,
      toFiniteNumber(trip.maxspeed, 0),
      toFiniteNumber(trip.maxSpeed, 0),
      toFiniteNumber(trip.topspeed, 0)
    ),
    0
  );
  const maxSpeedMps = pickFirstNonNegative(
    tripsData?.totalmaxspeed,
    mileageRecord?.maxspeed,
    mileageRecord?.maxSpeed,
    mileageRecord?.topspeed,
    mileageRecord?.highestspeed,
    additionalDetailReport?.totalmaxspeed,
    additionalDetailReport?.maxspeed,
    tripsMaxSpeed
  ) ?? 0;
  const avgSpeedMps = pickFirstNonNegative(
    mileageRecord?.avgspeed,
    mileageRecord?.avgspeed,
    mileageRecord?.averagespeed,
    mileageRecord?.meanspeed,
    additionalDetailReport?.totalaveragespeed,
    additionalDetailReport?.avgspeed,
    tripsData?.totalaveragespeed
  ) ?? 0;
  ...
  const tripParkingMs = trips.reduce(
    (sum: number, trip: Record<string, unknown>) => sum + Math.max(
      normalizeDurationMs(trip?.parking, dayWindowMs),
      normalizeDurationMs(trip?.parktime, dayWindowMs)
    ),
    0
  );
  const recordParkingMs = mileageRecord
    ? Math.max(
        0,
        normalizeDurationMs(mileageRecord.parkingduration, dayWindowMs),
        normalizeDurationMs(mileageRecord.parkduration, dayWindowMs),
        normalizeDurationMs(mileageRecord.parkingtime, dayWindowMs)
      )
    : 0;
  ...
  const additionalDetailParkingMs = additionalDetailReport
    ? Math.max(
        0,
        normalizeDurationMs(additionalDetailReport.parkingduration, dayWindowMs),
        normalizeDurationMs(additionalDetailReport.parkduration, dayWindowMs),
        normalizeDurationMs(additionalDetailReport.parkingtime, dayWindowMs),
        normalizeDurationMs(additionalDetailReport.totalpark, dayWindowMs),
        normalizeDurationMs(additionalDetailReport.totalparking, dayWindowMs),
        normalizeDurationMs(additionalDetailReport.parktime, dayWindowMs),
        normalizeDurationMs(additionalDetailReport.stopduration, dayWindowMs)
      )
    : 0;
  ...
  const parksExplicitParkingMs = additionalParkRecords.reduce(
    (sum: number, park: Record<string, unknown>) => {
      const explicit = Math.max(
        0,
        normalizeDurationMs(park.duration, dayWindowMs),
        normalizeDurationMs(park.parkingduration, dayWindowMs),
        normalizeDurationMs(park.parkduration, dayWindowMs),
        normalizeDurationMs(park.parkingtime, dayWindowMs),
        normalizeDurationMs(park.parktime, dayWindowMs),
        normalizeDurationMs(park.stopduration, dayWindowMs),
        normalizeDurationMs(park.durationidle, dayWindowMs)
      );
      return sum + explicit;
    },
    0
  ...
  const parkIdleMs = parks.reduce(
    (sum: number, park: Record<string, unknown>) => sum + Math.max(
      0,
      normalizeDurationMs(park?.durationidle, dayWindowMs),
      normalizeDurationMs(park?.idleduration, dayWindowMs),
      normalizeDurationMs(park?.idletime, dayWindowMs)
    ),
    0
  );
  const detailIdleMs = additionalDetailReport
    ? Math.max(
        0,
        normalizeDurationMs(additionalDetailReport.totalidle, dayWindowMs),
        normalizeDurationMs(additionalDetailReport.idleduration, dayWindowMs),
        normalizeDurationMs(additionalDetailReport.idletime, dayWindowMs),
        normalizeDurationMs(additionalDetailReport.totalstop, dayWindowMs)
      )
    : 0;
  ...
  const recordIdleMs = mileageRecord
    ? Math.max(
        0,
        normalizeDurationMs(mileageRecord.totalidle, dayWindowMs),
        normalizeDurationMs(mileageRecord.idleduration, dayWindowMs),
        normalizeDurationMs(mileageRecord.idletime, dayWindowMs)
      )
    : 0;
  const idleMsFromFields = Math.max(
    recordIdleMs,
    detailIdleMs,
    parkIdleMs,
    parsedIdleTextMs,
  ...
  const workingMs = explicitWorkingMs > 0 ? explicitWorkingMs : drivingMs + idleMsFromFields;
  // GPS51 idle typically overlaps with working when speed is zero, so do not subtract parking.
  const derivedIdleMs = Math.max(0, workingMs - drivingMs);
  const idleMs = idleMsFromFields > 0 ? idleMsFromFields : derivedIdleMs;
  ...
  const tripStartTime = pickTripTimestamp(trips, "start") ?? Number.MAX_SAFE_INTEGER;
  const tripEndTime = pickTripTimestamp(trips, "end") ?? 0;

  const firstPark = parks.length ? parks[0] : null;
  const lastPark = parks.length ? parks[parks.length - 1] : null;

  // Prefer per-day report boundaries first; these are aligned with the selected date.
  const reportStartTime = toOptionalTimestamp(mileageRecord?.starttime);
  const reportEndTime = toOptionalTimestamp(mileageRecord?.endtime);

  const startTimeValue = reportStartTime
  ...
  const endTimeValue = reportEndTime
    ?? (tripEndTime > 0
      ? tripEndTime
      : (lastTrackTime > 0
        ? lastTrackTime
        : toOptionalTimestamp(lastPark?.endtime)));

  const derivedParkingFromWindowMs =
    Number.isFinite(Number(startTimeValue)) && Number.isFinite(Number(endTimeValue))
      ? Math.max(0, Number(endTimeValue) - Number(startTimeValue) - drivingMs)
      : 0;
  const finalParkingMs = Math.max(resolvedParkingMs, derivedParkingFromWindowMs);
  ...
  ) || resolveAddressFromMap(
    tripAddressMap,
    Number(firstAdditionalRaw?.callat ?? firstAdditionalRaw?.silent ?? firstAdditionalRaw?.lat ?? firstTrackRaw?.callat ?? firstTrackRaw?.silent ?? firstTrackRaw?.lat),
    Number(firstAdditionalRaw?.callon ?? firstAdditionalRaw?.lng ?? firstAdditionalRaw?.lon ?? firstTrackRaw?.callon ?? firstTrackRaw?.lng ?? firstTrackRaw?.lon)
  ) || resolveAddressFromMap(
    tripAddressMap,
    Number(trips[0]?.slat),
    Number(trips[0]?.slon)
  ...
  const endAddress = pickReadableAddress(
    ...
    tripAddressMap,
    Number(lastAdditionalRaw?.callat ?? lastAdditionalRaw?.silent ?? lastAdditionalRaw?.lat ?? lastTrackRaw?.callat ?? lastTrackRaw?.silent ?? lastTrackRaw?.lat),
    Number(lastAdditionalRaw?.callon ?? lastAdditionalRaw?.lng ?? lastAdditionalRaw?.lon ?? lastTrackRaw?.callon ?? lastTrackRaw?.lng ?? lastTrackRaw?.lon)
  ) || resolveAddressFromMap(
    tripAddressMap,
    Number(trips[trips.length - 1]?.elat),
    Number(trips[trips.length - 1]?.elon)
  ...
  const firstTrip = trips.length ? trips[0] as Record<string, unknown> : null;
  const lastTrip = trips.length ? trips[trips.length - 1] as Record<string, unknown> : null;
  ...
  const startLat = pickCoordNum(firstAdditionalRaw, coordLatKeys)
    ?? pickCoordNum(firstTrackRaw, coordLatKeys)
    ?? pickTripCoordinate(firstTrip, ["slat", "startlat", "lat", "callat"]);
  const startLng = pickCoordNum(firstAdditionalRaw, coordLngKeys)
    ?? pickCoordNum(firstTrackRaw, coordLngKeys)
    ?? pickTripCoordinate(firstTrip, ["slon", "startlng", "startlon", "lng", "lon", "callon"]);
  const endLat = pickCoordNum(lastAdditionalRaw, coordLatKeys)
    ?? pickCoordNum(lastTrackRaw, coordLatKeys)
    ?? pickTripCoordinate(lastTrip, ["elat", "endlat", "lat", "callat"]);
  const endLng = pickCoordNum(lastAdditionalRaw, coordLngKeys)
    ?? pickCoordNum(lastTrackRaw, coordLngKeys)
    ?? pickTripCoordinate(lastTrip, ["elon", "endlng", "endlon", "lng", "lon", "callon"]);
  ...
  if (!hasExplicitTelemetryEvidence) {
    logGPS51("daily.summary.cache-fallback", {
      day,
      reason: "no explicit telemetry evidence",
      hasCachedSummary: Boolean(cachedSummary),
    });
    return cachedSummary;
  }
  ...
  const stableSummary = cachedSummary ? stabilizeDailySummary(cachedSummary, summary) : summary;
  setCachedDailySummary(day, stableSummary);
